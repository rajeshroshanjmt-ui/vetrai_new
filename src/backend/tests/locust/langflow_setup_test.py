#!/usr/bin/env python3
"""Vetrai Load Test Setup CLI

This script sets up a complete Vetrai test environment by:
1. Starting Vetrai (optional)
...
Usage:
    python setup_vetrai_test.py --help
    python setup_vetrai_test.py --interactive
    python setup_vetrai_test.py --flow "Basic Prompting"
    python setup_vetrai_test.py --list-flows
"""
...
async def get_starter_projects_from_api(host: str, access_token: str) -> list[dict]:
    """Get starter projects from Vetrai API."""
    import httpx
...
async def list_available_flows(host: str, access_token: str) -> list[tuple[str, str, str]]:
    """List all available starter project flows from Vetrai API.

    Args:
        host: Vetrai host URL
...
async def setup_vetrai_environment(host: str, flow_name: str | None = None, interactive: bool = False) -> dict:
    """Set up complete Vetrai environment with real starter project flows."""
    try:
        import httpx
    except ImportError:
    ...
    # Configuration - use default Vetrai credentials
    username = "vetrai"
    password = "vetrai"
    ...
    async with httpx.AsyncClient(base_url=host, timeout=60.0) as client:
        ...
        try:
            ...
            # Remove the id to let Vetrai generate a new one
...
def print_setup_results(setup_state: dict):
    ...
    print(f"export VETRAI_HOST='{setup_state['host']}'")
    ...
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-vetrai --headless --users 25 --duration 120"
    )
    print()

    print("# Generate HTML report:")
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-vetrai --headless --users 50 --duration 180 --html report.html"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Set up Vetrai load test environment with real starter project flows",
...
Examples:
  ...
  # Use specific flow
  python setup_vetrai_test.py --flow "Memory Chatbot"

  # List available flows
  python setup_vetrai_test.py --list-flows

  # Setup with custom host
  python setup_vetrai_test.py --host http://localhost:8000 --interactive

  # Save credentials to file
  python setup_vetrai_test.py --interactive --save-credentials test_creds.json
        """,
    )
        ...
        async def list_flows_only():
            ...
            async with httpx.AsyncClient(base_url=args.host, timeout=30.0) as client:
                ...
                try:
                    health_response = await client.get("/health")
                    if health_response.status_code != 200:
                        raise Exception(f"Vetrai not available at {args.host}")
                except Exception as e:
                    print(f"❌ Cannot connect to Vetrai at {args.host}: {e}")
                    sys.exit(1)
                ...
                try:
                    ...
                    print(f"❌ Authentication failed: {e}")
                    print("Make sure Vetrai is running with default credentials (vetrai/vetrai)")
                    sys.exit(1)
    ...
    try:
        ...
        setup_state = asyncio.run(
            setup_vetrai_environment(host=args.host, flow_name=args.flow, interactive=args.interactive)
        )
