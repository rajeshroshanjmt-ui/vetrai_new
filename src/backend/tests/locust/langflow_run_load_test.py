...
def wait_for_vetrai(host, timeout=60):
    ...
    while time.time() - start_time < timeout:
        if check_vetrai_running(host):
            print("✅ Vetrai is ready!")
            return True
        time.sleep(2)

    print(f"❌ Vetrai did not start within {timeout} seconds")
    return False
...
def start_vetrai(host, port):
    ...
    if check_vetrai_running(host):
        print(f"✅ Vetrai is already running at {host}")
        return None
    ...
    try:
        ...
        if not args.no_start_vetrai:
            ...
            if not vetrai_process:
                print("❌ Failed to start Vetrai")
                sys.exit(1)
        # Just check if it's running
        elif not check_vetrai_running(args.host):
            print(f"❌ Vetrai is not running at {args.host}")
            ...
            print(f"🔗 Using existing Vetrai instance at {args.host}")
            if args.host.startswith("https://"):
