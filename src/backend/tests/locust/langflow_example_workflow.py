#!/usr/bin/env python3
"""Example Vetrai Load Testing Workflow

This script demonstrates the complete workflow for setting up and running
Vetrai load tests with real starter project flows.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Example Vetrai Load Testing Workflow")
    parser.add_argument("--auto", action="store_true", help="Run automatically without user input prompts")
    args = parser.parse_args()

    print("🚀 Vetrai Load Testing Example Workflow")
    ...
    setup_script = script_dir / "vetrai_setup_test.py"
    runner_script = script_dir / "vetrai_run_load_test.py"
    ...
    try:
        ...
        success = run_command_with_env(
            [
                ...
                "--no-start-vetrai",
                "--html",
                "vetrai_load_test_report.html",
                "--csv",
                "vetrai_load_test_results",
            ],
        ...
        print("   python setup_vetrai_test.py --interactive")
        print()
        ...
        print("   - vetrai_load_test_report.html (detailed HTML report)")
        print("   - vetrai_load_test_results_*.csv (CSV data files)")
        print("   - vetrai_load_test_detailed_errors_*.log (detailed error logs)")
        print("   - vetrai_load_test_error_summary_*.json (error analysis)")
        print("   - vetrai_server_logs_during_test_*.log (Vetrai server logs)")
        print()
        print("🧹 Clean up:")
        print("   - Remove test flows from Vetrai UI")
        ...
        if args.auto:
            ...
            print("   - vetrai_load_test_report.html")
            print("   - vetrai_load_test_results_*.csv")
        else:
            ...
            if cleanup_response == "y":
                files_to_clean = [
                    "example_test_creds.json",
                    "vetrai_load_test_report.html",
                    "vetrai_load_test_results_failures.csv",
                    "vetrai_load_test_results_stats.csv",
                    "vetrai_load_test_results_stats_history.csv",
                    "vetrai_load_test_results_exceptions.csv",
                ]

                # Also clean up error logs (they have timestamps, so use glob pattern)
                import glob

                error_files = glob.glob("vetrai_load_test_detailed_errors_*.log")
                error_files.extend(glob.glob("vetrai_load_test_error_summary_*.json"))
                error_files.extend(glob.glob("vetrai_server_logs_during_test_*.log"))
        ...
        print("You're now ready to use Vetrai load testing for your own projects.")
        print()
        print("📊 View your load test results:")
        print("   • Open vetrai_load_test_report.html in your browser for detailed analysis")
        print("   • Check vetrai_load_test_results_*.csv for raw data")
        print("   • Review vetrai_load_test_detailed_errors_*.log for comprehensive error details")
        print("   • Analyze vetrai_load_test_error_summary_*.json for error patterns")
        print("   • Examine vetrai_server_logs_during_test_*.log for server-side issues")
