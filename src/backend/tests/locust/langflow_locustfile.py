"""Vetrai API Locust Load Testing File.

Comprehensive load testing for Vetrai API with multiple user behaviors and performance analysis.
...
Environment Variables:
    - VETRAI_HOST: Base URL for the Vetrai server (default: http://localhost:7860)
...
TEST_MESSAGES = {
    ...
    "realistic": "Hey, Could you check https://docs.vetrai.org for me? Later, could you calculate 1390 / 192 ?",
}
...
ERROR_LOG_FILE = None
VETRAI_LOG_FILE = None
DETAILED_ERRORS = []


def setup_error_logging():
    """Set up detailed error logging for the load test."""
    global ERROR_LOG_FILE, VETRAI_LOG_FILE

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Create detailed error log
    ERROR_LOG_FILE = f"vetrai_load_test_detailed_errors_{timestamp}.log"

    # Set up error logger
    error_logger = logging.getLogger("vetrai_load_test_errors")
    ...
    # Try to capture Vetrai logs
    vetrai_log_paths = ["vetrai.log", "logs/vetrai.log", "../../../vetrai.log", "../../../../vetrai.log"]

    for log_path in vetrai_log_paths:
        if Path(log_path).exists():
            VETRAI_LOG_FILE = log_path
            break

    print("📝 Error logging setup:")
    print(f"   • Detailed errors: {ERROR_LOG_FILE}")
    if VETRAI_LOG_FILE:
        print(f"   • Vetrai logs: {VETRAI_LOG_FILE}")
    else:
        print("   • Vetrai logs: Not found (will monitor common locations)")
...
def save_error_summary():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    summary_file = f"vetrai_load_test_error_summary_{timestamp}.json"
...
def capture_vetrai_logs():
    """Capture recent Vetrai logs if available."""
    if not VETRAI_LOG_FILE or not Path(VETRAI_LOG_FILE).exists():
        return None

    try:
        # Read last 1000 lines of Vetrai log
        with open(VETRAI_LOG_FILE) as f:
            lines = f.readlines()
            recent_lines = lines[-1000:] if len(lines) > 1000 else lines

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        captured_log_file = f"vetrai_server_logs_during_test_{timestamp}.log"

        with open(captured_log_file, "w") as f:
            f.write("# Vetrai server logs captured during load test\n")
            f.write(f"# Original log file: {VETRAI_LOG_FILE}\n")
            f.write(f"# Capture time: {datetime.now().isoformat()}\n")
            f.write(f"# Lines captured: {len(recent_lines)}\n\n")
            f.writelines(recent_lines)

        print(f"📋 Vetrai logs captured: {captured_log_file}")
        return captured_log_file

    except Exception as e:
        print(f"⚠️  Could not capture Vetrai logs: {e}")
        return None
...
def on_test_stop(environment, **_kwargs):
    ...
    print(f"\n{'=' * 60}")
    print(f"VETRAI API LOAD TEST RESULTS - GRADE: {grade}")
    print(f"{'=' * 60}")
...
class BaseVetraiUser(FastHttpUser):
    """Base class for all Vetrai API load testing user types."""

    abstract = True
    REQUEST_TIMEOUT = float(os.getenv("REQUEST_TIMEOUT", "30.0"))

    # Use the host provided by environment variable or default
    host = os.getenv("VETRAI_HOST", "http://localhost:7860")
    ...
    def on_start(self):
        ...
        if not self.api_key:
            raise ValueError("API_KEY environment variable is required. Run setup_vetrai_test.py first.")

        if not self.flow_id:
            raise ValueError("FLOW_ID environment variable is required. Run setup_vetrai_test.py first.")
        ...
        try:
            with self.client.post(
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        # Vetrai API success check - look for outputs
                        if data.get("outputs"):
...
class NormalUser(BaseVetraiUser):
...
class AggressiveUser(BaseVetraiUser):
...
class SustainedLoadUser(BaseVetraiUser):
...
class TailLatencyHunter(BaseVetraiUser):
...
class ScalabilityTestUser(BaseVetraiUser):
...
class BurstUser(BaseVetraiUser):
