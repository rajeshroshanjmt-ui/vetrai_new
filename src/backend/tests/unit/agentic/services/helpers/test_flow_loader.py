...
from vetrai.agentic.services.helpers.flow_loader import (
...
class TestValidatePathWithinBase:
    ...
    def test_should_return_resolved_path_for_valid_path(self, tmp_path):
        ...
        with patch("vetrai.agentic.services.helpers.flow_loader.FLOWS_BASE_PATH", tmp_path):
...
class TestLoadGraphFromPython:
    ...
    async def test_should_load_graph_from_get_graph_function(self):
        ...
        with (
            ...
            patch("vetrai.agentic.services.helpers.flow_loader._temporary_sys_path"),
        ):
            mock_spec = MagicMock()
...
class TestLoadGraphForExecution:
    ...
    async def test_should_use_python_loader_for_python_type(self):
        ...
        with patch(
            "vetrai.agentic.services.helpers.flow_loader._load_graph_from_python",
    ...
    async def test_should_use_json_loader_for_json_type(self):
        ...
        with (
            patch(
                "vetrai.agentic.services.helpers.flow_loader.load_and_prepare_flow",
                return_value='{"data": {"nodes": []}}',
            ) as mock_prepare,
            patch(
                "vetrai.agentic.services.helpers.flow_loader.aload_flow_from_json",
                new_callable=AsyncMock,
                return_value=mock_graph,
            ) as mock_load_json,
        ):
