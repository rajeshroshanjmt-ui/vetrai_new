...
class TestConfigure:
    ...
    @patch.dict(os.environ, {"VETRAI_LOG_LEVEL": "WARNING"})
    def test_configure_env_variable_override(self):
        """Test configure() respects VETRAI_LOG_LEVEL environment variable."""
    ...
    @patch.dict(os.environ, {"VETRAI_LOG_FILE": "/tmp/test.log"})  # noqa: S108
    def test_configure_env_log_file_override(self):
        """Test configure() respects VETRAI_LOG_FILE environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"VETRAI_LOG_ENV": "container"})
    def test_configure_env_log_env_override(self):
        """Test configure() respects VETRAI_LOG_ENV environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"VETRAI_LOG_FORMAT": "custom"})
    def test_configure_env_log_format_override(self):
        """Test configure() respects VETRAI_LOG_FORMAT environment variable."""
        configure()

        config = structlog._config
        assert config is not None

    @patch.dict(os.environ, {"VETRAI_PRETTY_LOGS": "false"})
    def test_configure_env_pretty_logs_disabled(self):
        """Test configure() respects VETRAI_PRETTY_LOGS=false."""
        configure()
...
def test_init_with_env_variable():
    with patch.dict(os.environ, {"VETRAI_LOG_RETRIEVER_BUFFER_SIZE": "100"}):
        buffer = SizedLogBuffer()
