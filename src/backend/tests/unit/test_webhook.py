...
import anyio
import pytest
from vetrai.services.event_manager import WebhookEventManager
...
async def test_vertex_builds_endpoint_returns_empty_for_new_flow(client, logged_in_headers):
    ...
    from vetrai.services.database.models.flow.model import FlowCreate
...
class TestGetVertexIdsFromFlow:
    ...
    def test_returns_empty_list_when_flow_data_is_none(self):
        ...
        from vetrai.api.v1.endpoints import _get_vertex_ids_from_flow
...
class TestSimpleRunFlowTask:
    ...
    async def test_emits_vertices_sorted_event_when_emit_events_true(self):
        ...
        from vetrai.api.v1.endpoints import simple_run_flow_task
        ...
        with (
            patch("vetrai.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run,
            patch("vetrai.api.v1.endpoints.webhook_event_manager") as mock_manager,
        ):
    ...
    async def test_emits_error_event_on_exception(self):
        ...
        with (
            patch("vetrai.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run,
            patch("vetrai.api.v1.endpoints.webhook_event_manager") as mock_manager,
            patch("vetrai.api.v1.endpoints.logger") as mock_logger,
        ):
    ...
    async def test_logs_telemetry_on_success(self):
        ...
        with patch("vetrai.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run:
            mock_run.return_value = {"result": "success"}
...
class TestWebhookEventsStreamAuth:
    ...
    async def test_calls_get_current_user_for_sse_for_authentication(self):
        ...
        from vetrai.api.v1.endpoints import webhook_events_stream
        ...
        with (
            patch("vetrai.api.v1.endpoints.get_current_user_for_sse", new_callable=AsyncMock) as mock_auth,
            patch("vetrai.api.v1.endpoints.webhook_event_manager") as mock_manager,
        ):
    ...
    async def test_raises_403_when_auth_fails(self):
        ...
        with patch("vetrai.api.v1.endpoints.get_current_user_for_sse", new_callable=AsyncMock) as mock_auth:
