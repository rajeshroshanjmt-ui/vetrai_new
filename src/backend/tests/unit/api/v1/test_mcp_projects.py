...
from vetrai.api.v1.mcp_projects import (
    ProjectMCPServer,
...
from vetrai.services.auth.utils import create_user_longterm_token, get_password_hash
from vetrai.services.database.models.flow import Flow
from vetrai.services.database.models.folder import Folder
from vetrai.services.database.models.user.model import User
from vetrai.services.deps import get_settings_service
...
@pytest.mark.parametrize(
    ("args", "urls", "expected"),
    [
        (None, ["https://vetrai.local/sse"], False),
        ([], ["https://vetrai.local/sse"], False),
        ([123, {"url": "foo"}], ["https://vetrai.local/sse"], False),
        (["https://vetrai.local/sse", 42], ["https://vetrai.local/sse"], True),
        (["alpha", "beta"], [], False),
    ],
)
...
def mock_project_mcp_server():
    with patch("vetrai.api.v1.mcp_projects.ProjectMCPServer") as mock:
...
def mock_sse_transport():
    with patch("vetrai.api.v1.mcp_projects.SseServerTransport") as mock:
...
def mock_streamable_http_manager():
    ...
    with patch("vetrai.api.v1.mcp_projects.StreamableHTTPSessionManager") as mock_class:
...
def mock_current_user_ctx(active_user):
    with patch("vetrai.api.v1.mcp_projects.current_user_ctx") as mock:
...
def mock_current_project_ctx(mock_project):
    with patch("vetrai.api.v1.mcp_projects.current_project_ctx") as mock:
...
def disable_mcp_composer_by_default():
    ...
    with patch("vetrai.api.v1.mcp_projects.get_settings_service") as mock_get_settings:
        from vetrai.services.deps import get_settings_service
...
async def test_update_project_auth_settings_encryption(
    ...
    from vetrai.services.auth.mcp_encryption import decrypt_auth_settings
...
async def test_project_sse_creation(user_test_project):
    ...
    assert mcp_server.server.name == f"vetrai-mcp-project-{project_id}"
...
async def test_project_session_manager_lifespan_handles_cleanup(user_test_project, monkeypatch):
    ...
    monkeypatch.setattr(
        "vetrai.api.v1.mcp_projects.StreamableHTTPSessionManager.run",
...
def _prepare_install_test_env(monkeypatch, tmp_path, filename="cursor.json"):
    ...
    monkeypatch.setattr("vetrai.api.v1.mcp_projects.get_client_ip", lambda request: "127.0.0.1")  # noqa: ARG005

    async def fake_get_config_path(client_name):  # noqa: ARG001
        return config_path

    monkeypatch.setattr("vetrai.api.v1.mcp_projects.get_config_path", fake_get_config_path)
    monkeypatch.setattr("vetrai.api.v1.mcp_projects.platform.system", lambda: "Linux")
    monkeypatch.setattr("vetrai.api.v1.mcp_projects.should_use_mcp_composer", lambda project: False)  # noqa: ARG005

    async def fake_streamable(project_id):
        return f"https://vetrai.local/api/v1/mcp/project/{project_id}/streamable"

    async def fake_sse(project_id):
        return f"https://vetrai.local/api/v1/mcp/project/{project_id}/sse"

    monkeypatch.setattr("vetrai.api.v1.mcp_projects.get_project_streamable_http_url", fake_streamable)
    monkeypatch.setattr("vetrai.api.v1.mcp_projects.get_project_sse_url", fake_sse)
    ...
    monkeypatch.setattr("vetrai.api.v1.mcp_projects.get_settings_service", lambda: dummy_service)
...
async def test_init_mcp_servers_error_handling():
    ...
    # Apply the patch
    with patch("vetrai.api.v1.mcp_projects.get_project_sse", side_effect=mock_get_project_sse):
...
async def test_init_mcp_servers_error_handling_streamable():
    ...
    # Apply the patch
    with patch("vetrai.api.v1.mcp_projects.get_project_mcp_server", side_effect=mock_get_project_mcp_server):
        # This should not raise any exception, as the error should be caught
        await init_mcp_servers()
