...
from vetrai.exceptions.api import WorkflowValidationError
...
class TestWorkflowDeveloperAPIProtection:
    ...
    def mock_settings_dev_api_disabled(self):
        ...
        with patch("vetrai.api.v2.workflow.get_settings_service") as mock_get_settings_service:
...
class TestWorkflowErrorHandling:
    ...
    async def test_database_error_returns_503(
        ...
        with patch("vetrai.api.v2.workflow.get_flow_by_id_or_endpoint_name") as mock_get_flow:
    ...
    async def test_execution_timeout_with_real_delay(
        ...
        try:
            ...
            with (
                patch("vetrai.api.v2.workflow.execute_sync_workflow", side_effect=slow_execution),
                patch("vetrai.api.v2.workflow.EXECUTION_TIMEOUT", 0.5),  # 0.5 second timeout
            ):
                headers = {"x-api-key": created_api_key.api_key}
                response = await client.post("api/v2/workflows", json=request_data, headers=headers)
    ...
    async def test_workflow_validation_error_propagation(
        ...
        try:
            ...
            with patch("vetrai.api.v2.workflow.execute_sync_workflow") as mock_execute:
...
class TestWorkflowSyncExecution:
    ...
    async def test_sync_execution_component_error_returns_200_with_error_in_body(
        ...
        try:
            ...
            with patch("vetrai.api.v2.workflow.run_graph_internal") as mock_run:
...
class TestWorkflowBackgroundQueueing:
    ...
    async def test_background_execution_flow(
        ...
        try:
            ...
            with (
                patch("vetrai.api.v2.workflow.get_task_service") as mock_get_task_service,
                patch("vetrai.api.v2.workflow.uuid4", return_value=UUID(mock_job_id)),
            ):
    ...
    async def test_background_execution_queue_exception(
        ...
        try:
            ...
            with patch("vetrai.api.v2.workflow.get_task_service") as mock_get_task_service:
                mock_task_service = MagicMock()
...
class TestWorkflowStatus:
    ...
    async def test_get_status_queued(
        ...
        with patch("vetrai.api.v2.workflow.get_job_service") as mock_get_job_service:
            mock_service = MagicMock()
    ...
    async def test_get_status_completed_reconstruction(
        ...
        with (
            patch("vetrai.api.v2.workflow.get_job_service") as mock_get_job_service,
            patch("vetrai.api.v2.workflow.get_flow_by_id_or_endpoint_name") as mock_get_flow,
            patch("vetrai.api.v2.workflow.reconstruct_workflow_response_from_job_id") as mock_reconstruct,
        ):
            mock_service = MagicMock()
            mock_service.get_job_by_job_id = AsyncMock(return_value=mock_job)
            mock_get_job_service.return_value = mock_service
