...
import pytest
import typer
from vetrai.__main__ import _create_superuser, app, get_number_of_workers
...
class TestSuperuserCommand:
    ...
    async def test_additional_superuser_requires_auth_production(self, client, active_super_user):  # noqa: ARG002
        ...
        with (
            patch("vetrai.services.deps.get_settings_service") as mock_settings,
            patch("vetrai.__main__.get_settings_service") as mock_settings2,
        ):
    ...
    async def test_failed_auth_token_validation(self, client, active_super_user):  # noqa: ARG002
        ...
        with (
            patch("vetrai.services.deps.get_settings_service") as mock_settings,
            patch("vetrai.__main__.get_settings_service") as mock_settings2,
            patch("vetrai.__main__.get_current_user_by_jwt", side_effect=Exception("Invalid token")),
            patch("vetrai.__main__.check_key", return_value=None),
        ):
            # Configure settings for production mode (AUTO_LOGIN=False)
            mock_auth_settings = type("MockAuthSettings", (), {"AUTO_LOGIN": False, "ENABLE_SUPERUSER_CLI": True})()
            mock_settings.return_value.auth_settings = mock_auth_settings
...
def test_get_number_of_workers():
    ...
    with (
        patch("vetrai.__main__.platform.system", return_value="Linux"),
        patch("vetrai.__main__.cpu_count", return_value=4),
    ):
