...
from lfx.interface.components import aget_all_types_dict, import_vetrai_components
...
class TestComponentLoading:
    ...
    @pytest.mark.asyncio
    async def test_import_vetrai_components_basic(self):
        """Test basic functionality of import_vetrai_components."""
        result = await import_vetrai_components()
    ...
    async def test_component_loading_performance_comparison(self, base_components_path):
        """Compare performance between import_vetrai_components and aget_all_types_dict."""
        # Warm up the functions (first calls might be slower due to imports)
        await import_vetrai_components()
        await aget_all_types_dict(base_components_path)

        # Time import_vetrai_components
        start_time = time.perf_counter()
        vetrai_result = await import_vetrai_components()
        vetrai_duration = time.perf_counter() - start_time
        ...
        print(f"import_vetrai_components: {vetrai_duration:.4f}s")
        print(f"aget_all_types_dict: {all_types_duration:.4f}s")
        print(f"Ratio (vetrai/all_types): {vetrai_duration / max(all_types_duration, 0.0001):.2f}")

        # Both should complete in reasonable time
        # Add warnings for slow performance before failing

        if vetrai_duration > 10.0:
            warnings.warn(f"import_vetrai_components is slow: {vetrai_duration:.2f}s", UserWarning, stacklevel=2)
        ...
        return {
            "vetrai_result": vetrai_result,
            "all_types_result": all_types_result,
            "vetrai_duration": vetrai_duration,
    ...
    async def test_result_structure_comparison(self, base_components_path):
        ...
        # Check vetrai result structure
        assert isinstance(vetrai_result, dict)
        assert "components" in vetrai_result
        vetrai_components = vetrai_result["components"]

        # Check all_types result structure
        assert isinstance(all_types_result, dict)

        # Get component counts (informational, non-failing)
        vetrai_count = sum(len(comps) for comps in vetrai_components.values())
        ...
        print(f"import_vetrai_components: {vetrai_count} components")
        print(f"aget_all_types_dict: {all_types_count} components")

        # Log the comparison but don't fail the test
        if vetrai_count != all_types_count:
            diff = abs(vetrai_count - all_types_count)
            print(f"Note: Component counts differ by {diff} - this may be due to OS file limits")

        # Analyze component categories
        if vetrai_components:
            vetrai_categories = list(vetrai_components.keys())
            print(f"Vetrai categories: {sorted(vetrai_categories)}")
        ...
        for category, components in vetrai_components.items():
    ...
    async def test_component_template_structure(self):
        ...
        for category, components in vetrai_result["components"].items():
            assert isinstance(components, dict), f"Category {category} should contain dict of components"
    ...
    async def test_concurrent_loading(self, base_components_path):
        ...
        tasks = [
            import_vetrai_components(),
            aget_all_types_dict(base_components_path),
            import_vetrai_components(),  # Run vetrai loader twice to test consistency
        ]
        ...
        vetrai_result1, all_types_result, vetrai_result2 = results

        print(f"\nConcurrent execution took: {concurrent_duration:.4f}s")

        # Check that both results have the same structure and component counts
        assert isinstance(vetrai_result1, dict)
        assert isinstance(vetrai_result2, dict)
        assert isinstance(all_types_result, dict)

        # Check that both vetrai results have the same component structure
        assert "components" in vetrai_result1
        assert "components" in vetrai_result2

        # Compare component counts (informational, non-failing)
        count1 = sum(len(comps) for comps in vetrai_result1["components"].values())
        count2 = sum(len(comps) for comps in vetrai_result2["components"].values())
        ...
        categories1 = set(vetrai_result1["components"].keys())
        categories2 = set(vetrai_result2["components"].keys())
        ...
        for category in categories1.intersection(categories2):
            comps1 = set(vetrai_result1["components"][category].keys())
            comps2 = set(vetrai_result2["components"][category].keys())
            if comps1 != comps2:
    ...
    async def test_memory_efficiency(self, base_components_path):
        ...
        # Load with import_vetrai_components
        vetrai_result = await import_vetrai_components()
        after_vetrai_objects = len(gc.get_objects())
        ...
        vetrai_objects_created = after_vetrai_objects - initial_objects
        all_types_objects_created = after_all_types_objects - after_vetrai_objects

        print("\nMemory Analysis:")
        print(f"Objects created by import_vetrai_components: {vetrai_objects_created}")
        print(f"Objects created by aget_all_types_dict: {all_types_objects_created}")

        # Clean up
        del vetrai_result, all_types_result
        gc.collect()
    ...
    async def test_error_handling(self):
        ...
        # import_vetrai_components should work regardless of external paths
        result = await import_vetrai_components()
    ...
    async def test_repeated_loading_performance(self, base_components_path):
        ...
        # Test repeated import_vetrai_components calls
        vetrai_times = []
        for _ in range(num_iterations):
            start_time = time.perf_counter()
            await import_vetrai_components()
            duration = time.perf_counter() - start_time
            vetrai_times.append(duration)
        ...
        vetrai_avg = sum(vetrai_times) / len(vetrai_times)
        vetrai_min = min(vetrai_times)
        vetrai_max = max(vetrai_times)
        ...
        print(
            f"import_vetrai_components - avg: {vetrai_avg:.4f}s, min: {vetrai_min:.4f}s, max: {vetrai_max:.4f}s"
        )
        print(f"aget_all_types_dict - avg: {all_types_avg:.4f}s, min: {all_types_min:.4f}s, max: {all_types_max:.4f}s")

        # Performance should be reasonably consistent
        vetrai_variance = max(vetrai_times) - min(vetrai_times)
        ...
        assert vetrai_variance < vetrai_avg * 10, (
            f"import_vetrai_components performance too inconsistent: {vetrai_variance}s variance"
        )
    ...
    async def test_comprehensive_performance_summary(self, base_components_path):
        ...
        await import_vetrai_components()  # Warm up imports, thread pools, etc.
        ...
        for run in range(num_runs):
            ...
            vetrai_result = await import_vetrai_components()
            vetrai_duration = time.perf_counter() - start_time
            vetrai_results.append((vetrai_duration, vetrai_result))
        ...
        vetrai_times = [duration for duration, _ in vetrai_results]
        all_types_times = [duration for duration, _ in all_types_results]

        print("\nSTEADY-STATE PERFORMANCE (after warm-up):")
        print("import_vetrai_components:")
        print(f"  Average: {sum(vetrai_times) / len(vetrai_times):.4f}s")
        print(f"  Min: {min(vetrai_times):.4f}s")
        print(f"  Max: {max(vetrai_times):.4f}s")
        ...
        vetrai_component_counts = []
        all_types_component_counts = []

        for _, result in vetrai_results:
            count = sum(len(comps) for comps in result.get("components", {}).values())
            vetrai_component_counts.append(count)
        ...
        print("\nCOMPONENT COUNTS:")
        print(f"import_vetrai_components: {vetrai_component_counts}")
        print(f"aget_all_types_dict: {all_types_component_counts}")

        # Determine which is faster (based on steady-state performance)
        avg_vetrai = sum(vetrai_times) / len(vetrai_times)
        avg_all_types = sum(all_types_times) / len(all_types_times)

        if avg_vetrai < avg_all_types:
            faster_method = "import_vetrai_components"
            speedup = avg_all_types / avg_vetrai
        else:
            faster_method = "aget_all_types_dict"
            speedup = avg_vetrai / avg_all_types

        print("\nSTEADY-STATE PERFORMANCE CONCLUSION:")
        print(f"Faster method: {faster_method}")
        print(f"Speedup factor: {speedup:.2f}x")
        print(f"Timing results: {avg_vetrai:.4f}s (vetrai), ", f"{avg_all_types:.4f}s (all_types)")
        ...
        if vetrai_component_counts:
            min_count = min(vetrai_component_counts)
            max_count = max(vetrai_component_counts)
        ...
        assert all(isinstance(result, dict) for _, result in vetrai_results), "All vetrai results should be dicts"
        ...
        print(f"Steady-state performance: avg_vetrai={avg_vetrai:.4f}s, speedup={speedup:.2f}x")
    ...
    async def test_component_differences_analysis(self, base_components_path):
        ...
        # import_vetrai_components returns {"components": {category: {comp_name: comp_data}}}
        # aget_all_types_dict returns {category: {comp_name: comp_data}}
        vetrai_components = vetrai_result.get("components", {})
        ...
        for category, components in vetrai_components.items():
            for comp_name in components:
                vetrai_flat[comp_name] = category
        ...
        # Calculate counts
        vetrai_count = len(vetrai_flat)
        all_types_count = len(all_types_flat)

        print("\nCOMPONENT COUNTS:")
        print(f"import_vetrai_components: {vetrai_count} components")
        print(f"aget_all_types_dict: {all_types_count} components")
        print(f"Difference: {abs(vetrai_count - all_types_count)} components")

        # Find components that are in one but not the other
        vetrai_only = set(vetrai_flat.keys()) - set(all_types_flat.keys())
        all_types_only = set(all_types_flat.keys()) - set(vetrai_flat.keys())
        common_components = set(vetrai_flat.keys()) & set(all_types_flat.keys())

        print("\nCOMPONENT OVERLAP:")
        print(f"Common components: {len(common_components)}")
        print(f"Only in import_vetrai_components: {len(vetrai_only)}")
        print(f"Only in aget_all_types_dict: {len(all_types_only)}")

        # Print detailed differences
        if vetrai_only:
            print(f"\nCOMPONENTS ONLY IN import_vetrai_components ({len(vetrai_only)}):")
            for comp_name in sorted(vetrai_only):
                category = vetrai_flat[comp_name]
        ...
        for comp_name in common_components:
            vetrai_cat = vetrai_flat[comp_name]
            all_types_cat = all_types_flat[comp_name]
            if vetrai_cat != all_types_cat:
                category_differences.append((comp_name, vetrai_cat, all_types_cat))

        if category_differences:
            print(f"\nCOMPONENTS WITH DIFFERENT CATEGORIES ({len(category_differences)}):")
            for comp_name, vetrai_cat, all_types_cat in sorted(category_differences):
                print(f"  - {comp_name}: import_vetrai='{vetrai_cat}' vs aget_all_types='{all_types_cat}'")

        # Print category summary
        print("\nCATEGORY SUMMARY:")
        vetrai_categories = set(vetrai_components.keys())
        all_types_categories = set(all_types_components.keys())

        print(f"Categories in import_vetrai_components: {sorted(vetrai_categories)}")
        print(f"Categories in aget_all_types_dict: {sorted(all_types_categories)}")

        categories_only_vetrai = vetrai_categories - all_types_categories
        categories_only_all_types = all_types_categories - vetrai_categories

        if categories_only_vetrai:
            print(f"Categories only in import_vetrai_components: {sorted(categories_only_vetrai)}")
        ...
        if vetrai_count == 0 and all_types_count == 0:
            print("Note: Both methods returned 0 components - this may be due to OS file limits")
        elif len(common_components) == 0 and (vetrai_count > 0 or all_types_count > 0):
    ...
    async def test_process_single_module_exception_handling(self):
        ...
        missing dependencies, or initialization failures), it doesn't crash Vetrai startup.
        """
        ...
        print("Component failures will not crash Vetrai startup")
        print("=" * 80)
