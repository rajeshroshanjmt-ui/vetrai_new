...
class TestS3CompatibleComponents:
    ...
    async def test_save_file_component_s3_upload(self, s3_settings):
        ...
        with (
            ...
            with (
                ...
                patch(
                    "vetrai.services.database.models.user.crud.get_user_by_id", new_callable=AsyncMock
                ) as mock_get_user,
                patch("vetrai.api.v2.files.upload_user_file", new_callable=AsyncMock) as mock_upload,
            ):
                mock_get_user.return_value = MagicMock()
                mock_upload.return_value = "s3_file.txt"

                # Test with DataFrame
                from vetrai.schema import Data, DataFrame
    ...
    async def test_components_work_in_local_mode(self, local_settings):
        ...
        with patch("vetrai.services.deps.get_settings_service", return_value=local_settings):
            component = FileComponent()
