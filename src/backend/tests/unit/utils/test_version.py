...
import httpx
import pytest
from vetrai.utils.version import (
...
class TestGetVersionInfo:
    ...
    @patch("vetrai.utils.version.metadata")
    def test_get_version_info_vetrai_package(self, mock_metadata):
        """Test getting version info for vetrai package."""
        ...
        assert result["package"] == "Vetrai"

    @patch("vetrai.utils.version.metadata")
    def test_get_version_info_vetrai_base_package(self, mock_metadata):
        """Test getting version info for vetrai-base package."""
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Vetrai Base"
    ...
    def test_get_version_info_nightly_package(self, mock_metadata):
        ...
        def mock_version(pkg_name):
            if pkg_name in ["vetrai", "vetrai-base"]:
                raise mock_metadata.PackageNotFoundError
            if pkg_name == "vetrai-nightly":
                return "1.0.0.dev456"
            raise mock_metadata.PackageNotFoundError
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Vetrai Nightly"
    ...
    def test_get_version_info_base_nightly_package(self, mock_metadata):
        ...
        def mock_version(pkg_name):
            if pkg_name in ["vetrai", "vetrai-base", "vetrai-nightly"]:
                raise mock_metadata.PackageNotFoundError
            if pkg_name == "vetrai-base-nightly":
                return "1.0.0.a1"
            raise mock_metadata.PackageNotFoundError
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Vetrai Base Nightly"
...
class TestFetchLatestVersion:
    ...
    @patch("vetrai.utils.version.httpx")
...
class TestGetVersionInfoFunction:
    ...
    @patch("vetrai.utils.version.VERSION_INFO")
    def test_get_version_info_returns_version_info(self, mock_version_info):
        """Test that get_version_info returns VERSION_INFO."""
        mock_version_info = {"version": "1.0.0", "main_version": "1.0.0", "package": "Vetrai"}

        with patch("vetrai.utils.version.VERSION_INFO", mock_version_info):
