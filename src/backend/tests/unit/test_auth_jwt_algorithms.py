...
class TestTokenCreation:
    ...
    def test_create_token_hs256(self):
        ...
        from vetrai.services.auth.utils import create_token

        with tempfile.TemporaryDirectory() as tmpdir:
            mock_service = self._create_mock_settings_service("HS256", tmpdir)

            with patch("vetrai.services.auth.utils.get_settings_service", return_value=mock_service):
                token = create_token(
...
class TestTokenVerification:
    ...
    async def test_verify_hs256_token_success(self):
        ...
        from vetrai.services.auth.utils import create_token, get_current_user_by_jwt
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("vetrai.services.auth.utils.get_settings_service", return_value=mock_service),
                patch("vetrai.services.auth.utils.get_user_by_id", return_value=mock_user),
            ):
                token = create_token(
                    data={"sub": "user-123", "type": "access"},
                    expires_delta=timedelta(hours=1),
...
class TestAuthenticationFailures:
    ...
    async def test_missing_public_key_rs256_raises_401(self):
        ...
        from vetrai.services.auth.utils import get_current_user_by_jwt
    ...
    async def test_user_not_found_raises_401(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("vetrai.services.auth.utils.get_settings_service", return_value=mock_service),
                patch("vetrai.services.auth.utils.get_user_by_id", return_value=None),
            ):
                token = create_token(
...
class TestRefreshTokenVerification:
    ...
    async def test_refresh_token_rs256_success(self):
        ...
        from vetrai.services.auth.utils import create_refresh_token, create_token
...
class TestJWTKeyHelpers:
    ...
    def test_get_jwt_verification_key_hs256_returns_secret_key(self):
        ...
        from vetrai.services.auth.utils import get_jwt_verification_key
    ...
    def test_get_jwt_verification_key_missing_public_key_raises_error(self):
        ...
        from vetrai.services.auth.utils import JWTKeyError, get_jwt_verification_key
    ...
    def test_get_jwt_signing_key_hs256_returns_secret_key(self):
        ...
        from vetrai.services.auth.utils import get_jwt_signing_key
    ...
    def test_verification_and_signing_keys_work_together_hs256(self):
        ...
        from vetrai.services.auth.utils import get_jwt_signing_key, get_jwt_verification_key
