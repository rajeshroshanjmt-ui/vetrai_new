...
from vetrai.main import create_app
from vetrai.services.auth.utils import get_password_hash
from vetrai.services.database.models.api_key.model import ApiKey, UnmaskedApiKeyRead
from vetrai.services.database.models.flow.model import Flow, FlowCreate, FlowRead
from vetrai.services.database.models.folder.model import Folder
from vetrai.services.database.models.transactions.model import TransactionTable
from vetrai.services.database.models.user.model import User, UserCreate, UserRead
from vetrai.services.database.models.vertex_builds.crud import delete_vertex_builds_by_flow_id
from vetrai.services.deps import get_db_service, session_scope
    ...
    if "benchmark" in request.keywords or "no_blockbuster" in request.keywords:
        ...
        with blockbuster_ctx() as bb:
            ...
            (
                ...
                .can_block_in("vetrai/custom/custom_component/component.py", "set_class_code")
...
async def _delete_transactions_and_vertex_builds(session, flows: list[Flow]):
    from vetrai.services.database.models.jobs.model import Job
...
def _setup_env(monkeypatch):
    monkeypatch.setenv("VETRAI_CACHE_TYPE", "redis")
    monkeypatch.setenv("VETRAI_REDIS_HOST", "result_backend")
    monkeypatch.setenv("VETRAI_REDIS_PORT", "6379")
    monkeypatch.setenv("VETRAI_REDIS_DB", "0")
    monkeypatch.setenv("VETRAI_REDIS_EXPIRE", "3600")
    monkeypatch.setenv("VETRAI_REDIS_PASSWORD", "")
...
def distributed_client_fixture(
    ...
    from vetrai.core import celery_app

    db_dir = tempfile.mkdtemp()
    try:
        db_path = Path(db_dir) / "test.db"
        monkeypatch.setenv("VETRAI_DATABASE_URL", f"sqlite:///{db_path}")
        monkeypatch.setenv("VETRAI_AUTO_LOGIN", "false")
        # monkeypatch vetrai.services.task.manager.USE_CELERY to True
        # monkeypatch.setattr(manager, "USE_CELERY", True)
        monkeypatch.setattr(celery_app, "celery_app", celery_app.make_celery("vetrai", Config))
...
def deactivate_tracing(monkeypatch):
    monkeypatch.setenv("VETRAI_DEACTIVATE_TRACING", "true")
    yield
    monkeypatch.undo()
...
def use_noop_session(monkeypatch):
    monkeypatch.setenv("VETRAI_USE_NOOP_DATABASE", "1")
    ...
    if "noclient" in request.keywords:
        ...
        def init_app():
            ...
            if "load_flows" in request.keywords:
                ...
                monkeypatch.setenv("VETRAI_LOAD_FLOWS_PATH", load_flows_dir)
                monkeypatch.setenv("VETRAI_AUTO_LOGIN", "true")
...
@pytest.fixture
def runner(tmp_path):
    env = {"VETRAI_DATABASE_URL": f"sqlite:///{tmp_path}/test.db"}
