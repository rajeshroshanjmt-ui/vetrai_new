...
def main():
    """Launches vetrai with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from vetrai.__main__ import main as vetrai_main

        vetrai_main()


def _launch_with_exec():
    """Launch vetrai by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "vetrai.__main__", *sys.argv[1:]])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec vetrai: {e}", file=sys.stderr)
        sys.exit(1)
