...
from vetrai.initial_setup.constants import (
...
from vetrai.services.database.models.folder.constants import (
    DEFAULT_FOLDER_DESCRIPTION,
    DEFAULT_FOLDER_NAME,
    LEGACY_FOLDER_NAMES,
)
from vetrai.services.database.models.folder.model import Folder, FolderCreate, FolderRead
from vetrai.services.deps import get_settings_service, get_storage_service, get_variable_service, session_scope
...
async def get_or_create_assistant_folder(session, user_id: UUID):
    """Create or get the Vetrai Assistant folder for a specific user.
...
async def create_or_update_agentic_flows(session: AsyncSession, user_id: UUID) -> None:
    """Create or update agentic flows in the Vetrai Assistant folder for a user.

    This function is called on user login to ensure that all agentic flows
    are present and up-to-date in the user's Vetrai Assistant folder.
    ...
    try:
        # Get or create the Vetrai Assistant folder
...
async def load_flows_from_directory() -> None:
    """On vetrai startup, this loads all flows from the directory specified in the settings.
...
async def initialize_auto_login_default_superuser() -> None:
    ...
    async with session_scope() as async_session:
        ...
        from vetrai.api.utils.mcp.agentic_mcp import initialize_agentic_user_variables
