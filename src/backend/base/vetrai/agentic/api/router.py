"""Vetrai Assistant API router.

This module provides the HTTP endpoints for the Vetrai Assistant.
...
from vetrai.agentic.api.schemas import AssistantRequest
from vetrai.agentic.services.assistant_service import (
    execute_flow_with_validation,
    execute_flow_with_validation_streaming,
)
from vetrai.agentic.services.flow_executor import execute_flow_file
from vetrai.agentic.services.flow_types import (
    VETRAI_ASSISTANT_FLOW,
    MAX_VALIDATION_RETRIES,
)
from vetrai.agentic.services.provider_service import (
    DEFAULT_MODELS,
    PREFERRED_PROVIDERS,
    check_api_key,
    get_enabled_providers_for_user,
)
from vetrai.api.utils.core import CurrentActiveUser, DbSession
from vetrai.services.deps import get_variable_service
...
async def _resolve_assistant_context(
    ...
    if not api_key:
        raise HTTPException(
            status_code=400,
            detail=(
                f"{api_key_name} is required for the Vetrai Assistant with {provider}. "
...
async def check_assistant_config(
    ...
    """Check if the Vetrai Assistant is properly configured.
...
async def assist(
    ...
    """Chat with the Vetrai Assistant."""
    ctx = await _resolve_assistant_context(request, current_user.id, session)

    logger.info(f"Executing {VETRAI_ASSISTANT_FLOW} with {ctx.provider}/{ctx.model_name}")

    return await execute_flow_with_validation(
        flow_filename=VETRAI_ASSISTANT_FLOW,
...
async def assist_stream(
    ...
    """Chat with the Vetrai Assistant with streaming progress updates."""
    ctx = await _resolve_assistant_context(request, current_user.id, session)
