...
from vetrai.middleware import ContentSizeLimitMiddleware
from vetrai.services.deps import (
    get_queue_service,
    get_service,
...
from vetrai.services.utils import initialize_services, initialize_settings_service, teardown_services
from vetrai.utils.mcp_cleanup import cleanup_mcp_sessions
...
def warn_about_future_cors_changes(settings):
    ...
    if using_defaults:
        logger.warning(
            ...
            "Set VETRAI_CORS_ORIGINS for production. Stricter defaults in v2.0."
        )
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            await logger.adebug(f"Starting Vetrai v{version}...")
        else:
            await logger.adebug("Starting Vetrai...")
        ...
        try:
            ...
            lock_file = Path(tempfile.gettempdir()) / "vetrai_starter_projects.lock"
            ...
            if get_settings_service().settings.agentic_experience:
                from vetrai.api.utils.mcp.agentic_mcp import initialize_agentic_global_variables
            ...
            if get_settings_service().settings.agentic_experience:
                from vetrai.api.utils.mcp.agentic_mcp import auto_configure_agentic_mcp_server
            ...
            from vetrai.api.v1.mcp import start_streamable_http_manager
            from vetrai.api.v1.mcp_projects import start_project_task_group
            ...
            if "vetrai migration --fix" not in str(exc):
            ...
            from vetrai.__main__ import get_number_of_workers
            from vetrai.cli.progress import create_vetrai_shutdown_progress

            log_level = os.getenv("VETRAI_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_vetrai_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(1):
                    from vetrai.api.v1.mcp import stop_streamable_http_manager
                    from vetrai.api.v1.mcp_projects import stop_project_task_group
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("Vetrai shutdown complete")
...
def create_app():
    ...
    if prome_port_str := os.environ.get("VETRAI_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from vetrai.api.v1 import mcp_router
...
def get_static_files_dir():
    """Get the static files directory relative to Vetrai's main.py file."""
...
# Module-level app for production deployment (uvicorn loads vetrai.main:app)
app = create_app()
