...
from vetrai.cli.progress import create_vetrai_progress
from vetrai.initial_setup.setup import get_or_create_default_folder
from vetrai.main import setup_app
from vetrai.services.auth.utils import check_key, get_current_user_by_jwt
from vetrai.services.deps import get_db_service, get_settings_service, is_settings_service_initialized, session_scope
from vetrai.services.utils import initialize_services
from vetrai.utils.version import fetch_latest_version, get_version_info
from vetrai.utils.version import is_pre_release as vetrai_is_pre_release
...
try:
    ...
    lfx_app = typer.Typer(name="lfx", help="Vetrai Executor commands")
...
def run(
    *,
    ...
    log_level = (log_level or os.environ.get("VETRAI_LOG_LEVEL") or "info").lower()
    os.environ["VETRAI_LOG_LEVEL"] = log_level
    ...
    progress = create_vetrai_progress(verbose=verbose)

    # Step 0: Initializing Vetrai
    ...
    if platform.system() == "Windows":
        ...
        with progress.step(6):
            # Use Gunicorn with VetraiUvicornWorker for non-Windows systems
            from vetrai.server import VetraiApplication
            ...
            server = VetraiApplication(app, options)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "vetrai")
        'A new version of vetrai is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=vetrai_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if vetrai_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use vetrai.utils.version to get the version info
    version_info = get_version_info()
    vetrai_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= vetrai_is_pre_release(vetrai_version)  # Update pre-release status

    notice = build_version_notice(vetrai_version, package_name)
    ...
    info_text = (
        f"{github_icon} GitHub: Star for updates {arrow} https://github.com/vetrai-ai/vetrai\n"
        f"{discord_icon} Discord: Join for support {arrow} https://discord.com/invite/EqksyE2EX9"
    )
    telemetry_text = (
        (
            "We collect anonymous usage data to improve Vetrai.\n"
            "To opt out, set: [bold]DO_NOT_TRACK=true[/bold] in your environment."
        )
        if os.getenv("DO_NOT_TRACK", os.getenv("VETRAI_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve Vetrai.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open Vetrai {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        fallback_message = (
            ...
            "* GitHub: https://github.com/vetrai-ai/vetrai\n"
            "# Discord: https://discord.com/invite/EqksyE2EX9\n\n"
            f"{telemetry_text}\n\n"
            f"[OK] Open Vetrai -> {protocol}://{access_host}:{port}"
        )
        try:
            ...
            logger.info("GitHub: https://github.com/vetrai-ai/vetrai")
            logger.info("Discord: https://discord.com/invite/EqksyE2EX9")
            logger.info(f"Open Vetrai: {protocol}://{access_host}:{port}")


@app.command()
def superuser(
    username: str = typer.Option(
        None, help="Username for the superuser. Defaults to 'vetrai' when AUTO_LOGIN is enabled."
    ),
    password: str = typer.Option(
        None, help="Password for the superuser. Defaults to 'vetrai' when AUTO_LOGIN is enabled."
    ),
    log_level: str = typer.Option("error", help="Logging level.", envvar="VETRAI_LOG_LEVEL"),
    auth_token: str = typer.Option(
        None, help="Authentication token of existing superuser.", envvar="VETRAI_SUPERUSER_TOKEN"
    ),
) -> None:
    """Create a superuser.
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set VETRAI_ENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from vetrai.services.database.models.user.crud import get_all_superusers
    ...
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            ...
            typer.echo("1. Set VETRAI_AUTO_LOGIN=false")
    ...
    # AUTO_LOGIN is false - production mode
        ...
        if not auth_token:
            ...
            typer.echo("To get a token, use: `uv run vetrai api_key`")
            raise typer.Exit(1)
    ...
    async with session_scope() as session:
        from vetrai.services.auth.utils import create_super_user

        if await create_super_user(db=session, username=username, password=password):
            # Verify that the superuser was created
            from vetrai.services.database.models.user.model import User
...
# command to copy the vetrai database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'vetrai.db' and 'vetrai-pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("vetrai"))
    db_path = cache_dir / "vetrai.db"
    pre_db_path = cache_dir / "vetrai-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from vetrai.services.database.models.api_key.crud import create_api_key, delete_api_key
            from vetrai.services.database.models.api_key.model import ApiKey, ApiKeyCreate
