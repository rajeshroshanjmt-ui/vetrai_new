...
from vetrai.services.auth.utils import get_current_active_user, get_current_active_user_mcp
from vetrai.services.database.models.flow.model import Flow
from vetrai.services.database.models.message.model import MessageTable
...
from vetrai.services.store.utils import get_lf_version_from_pypi
from vetrai.utils.constants import VETRAI_GLOBAL_VAR_HEADER_PREFIX

if TYPE_CHECKING:
    from vetrai.services.chat.service import ChatService
    from vetrai.services.store.schema import StoreComponentCreate
...
async def check_vetrai_version(component: StoreComponentCreate) -> None:
    ...
    vetrai_version = await get_lf_version_from_pypi()
    if vetrai_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Vetrai")
    if vetrai_version != component.last_tested_version:
        await logger.awarning(
            f"Your version of Vetrai ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({vetrai_version}) and try again."
        )
...
async def verify_public_flow_and_get_user(flow_id: uuid.UUID, client_id: str | None) -> tuple[User, uuid.UUID]:
    ...
    async with session_scope() as session:
        ...
        from vetrai.services.database.models.flow.model import AccessTypeEnum, Flow
    ...
    try:
        from vetrai.helpers.user import get_user_by_flow_id_or_endpoint_name
...
def extract_global_variables_from_headers(headers) -> dict[str, str]:
    """Extract global variables from HTTP headers with prefix X-VETRAI-GLOBAL-VAR-*.
    ...
    Example:
        headers = {"X-VETRAI-GLOBAL-VAR-API-KEY": "secret", "Content-Type": "application/json"}
    ...
    try:
        for header_name, header_value in headers.items():
            ...
            if header_lower.startswith(VETRAI_GLOBAL_VAR_HEADER_PREFIX):
                var_name = header_lower[len(VETRAI_GLOBAL_VAR_HEADER_PREFIX) :].upper()
