"""Utilities for auto-configuring the Vetrai Agentic MCP server."""
...
from vetrai.api.v2.mcp import get_server_list, update_server
from vetrai.services.database.models.user.model import User
from vetrai.services.deps import get_service, get_variable_service
from vetrai.services.schema import ServiceType
from vetrai.services.variable.constants import CREDENTIAL_TYPE, GENERIC_TYPE


async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the Vetrai Agentic MCP server for all users.

    This function adds the vetrai-agentic MCP server to each user's MCP
    ...
    await logger.ainfo("Auto-configuring Vetrai Agentic MCP server for all users...")
    ...
    try:
        ...
        server_name = "vetrai-agentic"
        python_executable = sys.executable
        server_config = {
            "command": python_executable,
            "args": ["-m", "vetrai.agentic.mcp"],
            "metadata": {
                "description": "Vetrai Agentic MCP server providing tools for flow/component operations, "
                "template search, and graph visualization",
                "auto_configured": True,
                "vetrai_internal": True,
            },
        }
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the Vetrai Agentic MCP server from all users.

    This function removes the vetrai-agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing Vetrai Agentic MCP server from all users...")
