...
from vetrai.api.v1.mcp import ResponseNoOp
from vetrai.api.v1.mcp_utils import (
...
from vetrai.services.auth.mcp_encryption import decrypt_auth_settings, encrypt_auth_settings
from vetrai.services.auth.utils import AUTO_LOGIN_WARNING
from vetrai.services.database.models import Flow, Folder
from vetrai.services.database.models.api_key.crud import check_key, create_api_key
from vetrai.services.database.models.api_key.model import ApiKey, ApiKeyCreate
from vetrai.services.database.models.user.crud import get_user_by_username
from vetrai.services.database.models.user.model import User
from vetrai.services.deps import get_service
...
async def verify_project_auth_conditional(
    ...
    async with session_scope() as session:
        ...
        from vetrai.services.auth.utils import get_current_user_mcp
...
async def install_mcp_config(
    ...
    try:
        ...
        if use_mcp_composer:
            ...
            command = "uvx"
            args = ["mcp-proxy"]
            # Check if we need to add Vetrai API key headers
            # Necessary only when Project API Key Authentication is enabled

            # Generate a Vetrai API key for auto-install if needed
            ...
            if should_generate_api_key:
                async with session_scope() as api_key_session:
                    ...
                    vetrai_api_key = api_key_response.api_key
                    args.extend(["--headers", "x-api-key", vetrai_api_key])
        ...
        # Remove stale entries that point to the same Vetrai URLs (e.g. after the project is renamed)
...
class ProjectMCPServer:
    def __init__(self, project_id: UUID):
        ...
        self.server = Server(f"vetrai-mcp-project-{project_id}")
...
async def register_project_with_composer(project: Folder):
    ...
    try:
        ...
        if not settings.host or not settings.port:
            error_msg = "Vetrai host and port must be set in settings to register project with MCP Composer"
            raise ValueError(error_msg)
