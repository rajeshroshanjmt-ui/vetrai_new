...
from vetrai.api.v1.endpoints import simple_run_flow
from vetrai.api.v1.schemas import SimplifiedAPIRequest
from vetrai.helpers.flow import json_schema_from_flow
...
from vetrai.services.deps import get_settings_service, get_storage_service, session_scope
...
# Carries per-request variables injected via HTTP headers (e.g., X-Vetrai-Global-Var-*)
            ...
            if current_user:
                ...
                for user_file in user_files:
                    ...
                    if stored_filename.startswith(f"{MCP_SERVERS_FILE}_{current_user.id}"):
                        # reserved file name for vetrai MCP server config file(s)
                        continue
