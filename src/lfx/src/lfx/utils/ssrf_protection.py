...
Configuration:
    VETRAI_SSRF_PROTECTION_ENABLED: Enable/disable SSRF protection (default: false)
        TODO: Change default to true in next major version (2.0)
    VETRAI_SSRF_ALLOWED_HOSTS: Comma-separated list of allowed hosts/CIDR ranges
        Examples: "192.168.1.0/24,internal-api.company.local,10.0.0.5"

TODO: In next major version (2.0):
    - Change VETRAI_SSRF_PROTECTION_ENABLED default to "true"
...
def _validate_direct_ip_address(hostname: str) -> bool:
    ...
    if is_ip_blocked(ip_obj):
        msg = (
            ...
            "To allow this IP, add it to VETRAI_SSRF_ALLOWED_HOSTS environment variable."
        )
        raise SSRFProtectionError(msg)
...
def _validate_hostname_resolution(hostname: str) -> None:
    ...
    if blocked_ips:
        msg = (
            ...
            "To allow this hostname, add it to VETRAI_SSRF_ALLOWED_HOSTS environment variable."
        )
        raise SSRFProtectionError(msg)
