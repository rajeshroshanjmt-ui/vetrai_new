...
_VETRAI_NAMESPACE_UUID = uuid.UUID("3c091057-e799-4e32-8ebc-27bc31e1108c")
...
def get_api_key() -> str:
    ...
    api_key = os.getenv("VETRAI_API_KEY")
    if not api_key:
        msg = "VETRAI_API_KEY environment variable is not set"
...
def flow_id_from_path(file_path: Path, root_dir: Path) -> str:
    ...
    return str(uuid.uuid5(_VETRAI_NAMESPACE_UUID, relative))
