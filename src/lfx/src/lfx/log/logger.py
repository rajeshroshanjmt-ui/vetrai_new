"""Logging configuration for Vetrai using structlog."""
...
class SizedLogBuffer:
    ...
    def __init__(
        ...
        The buffer can be overwritten by an env variable VETRAI_LOG_RETRIEVER_BUFFER_SIZE
    ...
    def max(self) -> int:
        ...
        if self._max == 0:
            env_buffer_size = os.getenv("VETRAI_LOG_RETRIEVER_BUFFER_SIZE", "0")
...
def configure(
    *,
    ...
    if os.getenv("VETRAI_LOG_LEVEL", "").upper() in VALID_LOG_LEVELS and log_level is None:
        log_level = os.getenv("VETRAI_LOG_LEVEL")

    log_level_str = os.getenv("VETRAI_LOG_LEVEL", "ERROR")
    ...
    if log_file is None:
        env_log_file = os.getenv("VETRAI_LOG_FILE", "")
        log_file = Path(env_log_file) if env_log_file else None

    if log_env is None:
        log_env = os.getenv("VETRAI_LOG_ENV", "")

    # Get log format from env if not provided
    if log_format is None:
        log_format = os.getenv("VETRAI_LOG_FORMAT")
    ...
    # Add callsite information only when VETRAI_DEV is set
    if DEV:
        processors.append(
    ...
    if log_env.lower() == "container" or log_env.lower() == "container_json":
        ...
        log_stdout_pretty = os.getenv("VETRAI_PRETTY_LOGS", "true").lower() == "true"
