...
class Settings(BaseSettings):
    # Define the default VETRAI_DIR
    config_dir: str | None = None
    # Define if vetrai db should be saved in config dir or
    # in the vetrai directory
    save_db_in_config_dir: bool = False
    """Define if vetrai database should be saved in VETRAI_CONFIG_DIR or in the vetrai directory
    (i.e. in the package directory)."""

    knowledge_bases_dir: str | None = "~/.vetrai/knowledge_bases"
    """The directory to store knowledge bases."""

    dev: bool = False
    """If True, Vetrai will run in development mode."""
    database_url: str | None = None
    """Database URL for Vetrai. If not provided, Vetrai will use a SQLite database.
    ...
    """If True, Vetrai will retry to connect to the database if it fails."""
    pool_size: int = 20
    ...
    If not provided, a hash of the database URL will be used. Useful when multiple Vetrai
    ...
    Controlled by VETRAI_USE_NOOP_DATABASE env variable."""
    ...
    """If set to True, Vetrai will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which Vetrai will expose Prometheus metrics. 9090 is the default port."""
    ...
    store_url: str | None = "https://api.vetrai.store"
    download_webhook_url: str | None = "https://api.vetrai.store/flows/trigger/ec611a61-8460-4438-b187-a4f65e5559d4"
    like_webhook_url: str | None = "https://api.vetrai.store/flows/trigger/64275852-ec00-45c1-984e-3bff814732da"

    storage_type: str = "local"
    """Storage type for file storage. Defaults to 'local'. Supports 'local' and 's3'."""
    object_storage_bucket_name: str | None = "vetrai-bucket"
    """Object storage bucket name for file storage. Defaults to 'vetrai-bucket'."""
    ...
    with the same name in case Vetrai fails to retrieve the variable value."""
    ...
    user_agent: str = "vetrai"
    """User agent for the API calls."""
    backend_only: bool = False
    """If set to True, Vetrai will not serve the frontend."""
    ...
    """If set to True, Vetrai will not track telemetry."""
    telemetry_base_url: str = "https://vetrai.gateway.scarf.sh"
    transactions_storage_enabled: bool = True
    """If set to True, Vetrai will track transactions between flows."""
    vertex_builds_storage_enabled: bool = True
    """If set to True, Vetrai will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "localhost"
    """The host on which Vetrai will run."""
    port: int = 7860
    """The port on which Vetrai will run."""
    ...
    """The log level for Vetrai."""
    log_file: str | None = "logs/vetrai.log"
    """The path to log file for Vetrai."""
    ...
    """If set to True, Vetrai will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, Vetrai will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which Vetrai will auto save flows."""
    ...
    """If set to False, Vetrai will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, Vetrai will not send progress notifications in the MCP server."""
    ...
    """If set to False, Vetrai will not start the MCP Composer service."""
    ...
    """If set to True, Vetrai will start the agentic MCP server that provides tools for
    flow/component operations, template search, and graph visualization."""

    # Developer API
    developer_api_enabled: bool = False
    """If set to True, Vetrai will enable developer API endpoints for advanced debugging and introspection."""
    ...
    """If set to True, Vetrai will only partially load components at startup and fully load them on demand.
    ...
    """If set to True, Vetrai will create starter projects. If False, skips all starter project setup.
    ...
    """If set to True, Vetrai will update starter projects."""

    # SSRF Protection
    ssrf_protection_enabled: bool = False
    """If set to True, Vetrai will enable SSRF (Server-Side Request Forgery) protection.
    ...
    def set_variables_to_get_from_environment(cls, value):
        ...
        if os.getenv("VETRAI_AGENTIC_EXPERIENCE", "true").lower() == "true":
    ...
    def set_vetrai_dir(cls, value):
        if not value:
            ...
            # Create a .vetrai directory inside the cache directory
    ...
    def set_database_url(cls, value, info):
        ...
        if vetrai_database_url := os.getenv("VETRAI_DATABASE_URL"):
            value = vetrai_database_url
            logger.debug("Using VETRAI_DATABASE_URL env variable")
        else:
            # Originally, we used sqlite:///./vetrai.db
            ...
            from lfx.utils.version import is_pre_release as vetrai_is_pre_release

            version = get_version_info()["version"]
            is_pre_release = vetrai_is_pre_release(version)

            if info.data["save_db_in_config_dir"]:
                database_dir = info.data["config_dir"]
            else:
                # Use vetrai package path, not lfx, for backwards compatibility
                try:
                    import vetrai

                    database_dir = Path(vetrai.__file__).parent.resolve()
                except ImportError:
                    database_dir = Path(__file__).parent.parent.parent.resolve()

            pre_db_file_name = "vetrai-pre.db"
            db_file_name = "vetrai.db"
    ...
    def set_components_path(cls, value):
        ...
        If the `VETRAI_COMPONENTS_PATH` environment variable is set and points to an existing path, it is
        appended to the provided list if not already present. If the input list is empty or missing, it is
        set to an empty list.
        """
        if os.getenv("VETRAI_COMPONENTS_PATH"):
            logger.debug("Adding VETRAI_COMPONENTS_PATH to components_path")
            vetrai_component_path = os.getenv("VETRAI_COMPONENTS_PATH")
            if Path(vetrai_component_path).exists() and vetrai_component_path not in value:
                if isinstance(vetrai_component_path, list):
                    for path in vetrai_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {vetrai_component_path} to components_path")
                elif vetrai_component_path not in value:
                    value.append(vetrai_component_path)
                    logger.debug(f"Appending {vetrai_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="VETRAI_")
