...
async def emit_vertex_build_event(
    *,
    ...
    try:
        ...
        from vetrai.services.event_manager import webhook_event_manager
        ...
        pass  # vetrai not available (standalone lfx usage)
...
async def log_transaction(
    ...
    Uses the pluggable TransactionService to log transactions. When running within vetrai,
...
async def log_vertex_build(
    *,
    ...
    When running within vetrai, it will use vetrai's database service to persist the build.
    When running standalone (lfx only), it will only log debug messages.
    """
    try:
        # Try to use vetrai's services if available (when running within vetrai)
        try:
            from vetrai.services.deps import get_db_service as vetrai_get_db_service
            from vetrai.services.deps import get_settings_service as vetrai_get_settings_service

            settings_service = vetrai_get_settings_service()
            ...
            from vetrai.services.database.models.vertex_builds.crud import (
                log_vertex_build as crud_log_vertex_build,
            )
            from vetrai.services.database.models.vertex_builds.model import VertexBuildBase
            ...
            db_service = vetrai_get_db_service()
            ...
            # Fallback for standalone lfx usage (without vetrai)
            ...
            # Log basic vertex build info - concrete implementation is in vetrai
