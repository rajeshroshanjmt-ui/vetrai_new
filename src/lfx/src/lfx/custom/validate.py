...
_VETRAI_IS_INSTALLED = False

with contextlib.suppress(ImportError):
    import vetrai  # noqa: F401

    _VETRAI_IS_INSTALLED = True
...
def validate_code(code):
    ...
    # Evaluate the function definition with vetrai context
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            code_obj = compile(ast.Module(body=[node], type_ignores=[]), "<string>", "exec")
            try:
                # Create execution context with common vetrai imports
                exec_globals = _create_vetrai_execution_context()
...
def _create_vetrai_execution_context():
    """Create execution context with common vetrai imports."""
    context = {}

    # Import common vetrai types that are used in templates
    try:
...
def create_class(code, class_name):
    ...
    code = code.replace("from vetrai import CustomComponent", "from vetrai.custom import CustomComponent")
    code = code.replace(
        "from vetrai.interface.custom.custom_component import CustomComponent",
        "from vetrai.custom import CustomComponent",
    )
...
def prepare_global_scope(module):
    ...
    for node in import_froms:
        ...
        # If original module starts with vetrai, also try lfx equivalent
        if node.module and node.module.startswith("vetrai."):
            lfx_module_name = node.module.replace("vetrai.", "lfx.", 1)
...
def get_default_imports(code_string):
    ...
    vetrai_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, vetrai_imports)
    vetrai_module = importlib.import_module("lfx.field_typing")
    default_imports.update({name: getattr(vetrai_module, name) for name in necessary_imports})
