...
def _get_cache_path() -> Path:
    ...
    cache_dir = Path(user_cache_dir("lfx", "vetrai"))
...
def _save_generated_index(modules_dict: dict) -> None:
    ...
    try:
        ...
        vetrai_version = version("vetrai")

        # Build index structure
        index = {
            "version": vetrai_version,
            "metadata": {
...
async def _send_telemetry(
    ...
    try:
        ...
        from vetrai.services.telemetry.schema import ComponentIndexPayload
...
async def _load_components_dynamically(
    ...
    try:
        ...
        await logger.aerror(f"Failed to import vetrai.components package: {e}", exc_info=True)
        return modules_dict
...
async def import_vetrai_components(
    settings_service: Optional["SettingsService"] = None,
    telemetry_service: Any | None = None,
) -> dict[str, dict[str, Any]]:
    """Asynchronously discovers and loads all built-in Vetrai components.
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in Vetrai
    ...
    if component_cache.all_types_dict is None:
        ...
        vetrai_components = await import_vetrai_components(settings_service, telemetry_service)
