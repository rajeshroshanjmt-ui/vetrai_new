...
class TestStandaloneLFX:
    """Test LFX running standalone without vetrai."""
...
class TestLFXWithVetraiConfig:
    """Test LFX with vetrai configuration."""

    @pytest.fixture
    def vetrai_config_dir(self, tmp_path):
        """Create a temporary vetrai-style config directory."""
        config_dir = tmp_path / "config"
        config_dir.mkdir()

        # Create lfx.toml with vetrai services
...
"""
    ...
    def test_config_overrides_defaults(self, clean_manager, vetrai_config_dir):
        """Test that config file overrides default services."""
        # Discover plugins from config
        clean_manager.discover_plugins(vetrai_config_dir)
    ...
    def test_multiple_services_from_config(self, clean_manager, vetrai_config_dir):
