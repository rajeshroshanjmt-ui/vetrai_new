...
"""
        ...
        # Verify dependency analysis results
        dep_info = mock_frontend.metadata["dependencies"]
        assert dep_info["total_dependencies"] == 1  # Only vetrai (os is stdlib, filtered out)

        # Check for dependencies
        package_names = [pkg["name"] for pkg in dep_info["dependencies"]]
        assert "vetrai" in package_names  # vetrai should be detected as external
...
from vetrai.base.models.model import LCModelComponent
from vetrai.field_typing import LanguageModel
...
"""
        ...
        build_component_metadata(
            mock_frontend, test_component, "vetrai.components.lmstudio", "LMStudioModelComponent"
        )
        ...
        assert "vetrai" in package_names  # project dependency
