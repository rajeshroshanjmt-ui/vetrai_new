...
@pytest.fixture
def vetrai_cache_dir(tmp_path):
    """Create a temporary vetrai cache directory."""
    cache_dir = tmp_path / "vetrai"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(vetrai_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = vetrai_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("vetrai"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="Vetrai")
    assert message.text == "Hello, Vetrai!"
...
def test_message_with_multiple_images(sample_image, vetrai_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("vetrai")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
