...
def test_get_api_key_missing():
    ...
    with (
        ...
        pytest.raises(
            ValueError,
            match="VETRAI_API_KEY environment variable is not set",
        ),
    ):
        get_api_key()
...
def test_create_multi_serve_app_single_flow(mock_graph, test_flow_meta):
    ...
    with patch.dict(os.environ, {"VETRAI_API_KEY": "test-key"}):  # pragma: allowlist secret
...
def test_serve_command_json_file():
    ...
    try:
        ...
        with (
            ...
            patch.dict(os.environ, {"VETRAI_API_KEY": "test-key"}),  # pragma: allowlist secret
        ):
            import typer
