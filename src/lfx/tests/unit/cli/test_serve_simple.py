...
def test_serve_command_missing_api_key():
    ...
    try:
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert "VETRAI_API_KEY" in str(result.output or result.exception or "")
    finally:
...
def test_serve_command_with_flow_json():
    ...
    with patch.dict(os.environ, {"VETRAI_API_KEY": "test-key"}), patch("uvicorn.run") as mock_uvicorn:
        runner = CliRunner()
...
def test_serve_command_invalid_json():
    ...
    with patch.dict(os.environ, {"VETRAI_API_KEY": "test-key"}):
        runner = CliRunner()
