...
def pytest_configure(config):  # noqa: ARG001
    ...
    # Check if vetrai is installed first - fail fast
    import os

    if not os.getenv("LFX_TEST_ALLOW_VETRAI"):
        try:
            ...
            pytest.exit(
                "\n"
                "=" * 80 + "\n"
                "ERROR: vetrai is installed. These tests require vetrai to NOT be installed.\n"
                "\n"
                ...
                "    uv run pytest ...\n"
                "\n"
                "The lfx tests are designed to run in isolation from vetrai to ensure proper\n"
                "packaging and dependency management.\n"
                "=" * 80 + "\n",
                returncode=1,
            )
        except ImportError:
            # Good, vetrai is not installed
            pass
...
# Simple client fixture for basic HTTP testing (without full vetrai app dependencies)
@pytest.fixture(name="client")
async def simple_client_fixture():
    """Simple HTTP client for basic testing."""
    # For lfx-specific tests, we might not need the full vetrai app
