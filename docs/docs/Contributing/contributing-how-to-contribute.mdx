...
This guide is intended to help you start contributing to Vetrai.
As an open-source codebase in a rapidly developing field, Vetrai welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.

To contribute code or documentation to Vetrai, follow the [pull request guide](#open-a-pull-request).

## Vetrai services overview

This overview will help you understand how to set up your development environment.

Vetrai consists of two main services:
...
When you install Vetrai as an end user, the backend serves the frontend internally, making both services available on a single port (`7860` by default).

## Install Vetrai from source

Install Vetrai from source by forking the repository and setting up your development environment.
...
### Clone the Vetrai repository

1. Fork the [Vetrai GitHub repository](https://github.com/vetrai-ai/vetrai).

2. Add the new remote to your local repository on your local machine:
```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/vetrai.git
```
Replace the following:
* `FORK_NAME`: A name for your fork of the repository
* `GIT_USERNAME`: Your Git username

### Run Vetrai from source

You can run Vetrai from source after cloning the repository, even if you aren't contributing to the codebase.
This builds the frontend and serves it through the backend on port `7860`.

The instructions below are for running Vetrai from source. For development with hot reload, see [Set up development environment](#set-up-your-vetrai-development-environment).

<details>
<summary>Run from source on macOS/Linux</summary>

In your terminal, navigate to the root of the Vetrai directory, and then run `make run_cli`.
...
The Vetrai frontend is served at `http://localhost:7860`.
</details>

<details>
<summary>Run from source with Windows CMD</summary>

To run Vetrai from source on Windows, you can use the Vetrai repository's included scripts, or run the commands in the terminal.

Do one of the following:

* To install and run Vetrai with the included Windows Batch file, navigate to the `scripts/windows` directory, and then run the `build_and_run.bat` file.

* To run Vetrai from the Windows Command Line:
    ...
    2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/vetrai/frontend`.
...
* To install and run Vetrai with the included scripts, navigate to the `scripts/windows` directory, and then run the `build_and_run.ps1` file.

* To run Vetrai from a PowerShell terminal:
...
### Set up your Vetrai development environment
...
If you just want to run Vetrai locally without making code changes, see [Run Vetrai from source](#run-vetrai-from-source).

<Tabs>
<TabItem value="linux-macos" label="Linux or macOS" default>

1. Set up the Vetrai development environment:
...
Since Windows doesn't include `make`, building and running Vetrai from source uses `npm` and `uv`.

To set up the Vetrai development environment, run the frontend and backend in separate terminals:
...
If you encounter frontend build problems or are upgrading from an older version of Vetrai, run `make run_clic` once.
...
2. Fork the [Vetrai GitHub repository](https://github.com/vetrai-ai/vetrai).
...
4. From the root of your local Vetrai fork, change to the `/docs` directory:
...
6. To edit and create content, work with the `.mdx` files in the `vetrai/docs/docs` directory.

    Create new files in `.mdx` format.

    Navigation is defined in `vetrai/docs/sidebars.js`.
...
* Pull request titles appear in Vetrai's release notes, so they should explain what the PR does as explicitly as possible.
* Pull requests should strive to fix one thing **only**, and should contain a good description of what is being fixed.

3. A Vetrai maintainer will review your pull request and may request changes, so ensure you pay attention to your PRs. Thanks for your contribution!
