...
This tutorial demonstrates how you can use Vetrai to create a chatbot application that uses Retrieval Augmented Generation (RAG) to embed your data as vectors in a vector database, and then chat with the data.
...
* Install the [Vetrai JavaScript client](/typescript-client)
* Be familiar with vector search concepts and applications, such as vector databases and RAG

## Create a vector RAG flow

1. In Vetrai, click **New Flow**, and then select the **Vector Store RAG** template.
...
In situations where many users load data or you need to load data programmatically, use the Vetrai API option.
...
2. Select the local file you want to upload, and then click **Open**.
    The file is loaded to your Vetrai server.
...
<TabItem value="API" label="Vetrai API">

To load data programmatically, use the `/v2/files/` and `/v1/run/$FLOW_ID` endpoints. The first endpoint loads a file to your Vetrai server, and then returns an uploaded file path. The second endpoint runs the **Load Data Flow**, referencing the uploaded file path, to chunk, embed, and load the data into the vector store.

The following script demonstrates this process.
For help with creating this script, use the [Vetrai File Upload Utility](https://vetrai-file-upload-examples.21cgwws14kdz.us-east.codeengine.appdomain.cloud/).
...
const headers = { 'x-api-key': 'VETRAI_API_KEY' };

// 2. Upload the file to Vetrai
const uploadRes = await fetch('VETRAI_SERVER_ADDRESS/api/v2/files/', {
  method: 'POST',
  headers,
  body: data
});
...
// 3. Call the Vetrai run endpoint with the uploaded file path
const payload = {
...
const runRes = await fetch('VETRAI_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-api-key': 'VETRAI_API_KEY' },
  body: JSON.stringify(payload)
});
const vetraiData = await runRes.json();
// Output only the message
console.log(vetraiData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const { VetraiClient } = require('@datastax/vetrai-client');

    # pragma: allowlist nextline secret
    const API_KEY = 'VETRAI_API_KEY';
    const SERVER = 'VETRAI_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';

    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });

    // Initialize the Vetrai client
    const client = new VetraiClient({
        baseUrl: SERVER,
        apiKey: API_KEY
    });
    ...
    function chat() {
        console.log('🤖 Vetrai RAG Chatbot (type "quit" to exit)\n');
    ...
    :::tip
    The [Vetrai TypeScript client](/typescript-client) has a `chatOutputText()` convenience method that simplifies working with Vetrai's complex JSON response structure.
