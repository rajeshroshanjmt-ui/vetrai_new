...
This tutorial shows you how to connect a JavaScript application to a [Vetrai agent](/agents).
...
In this example, the application sends a customer's email address to the Vetrai agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    const VETRAI_SERVER_ADDRESS = 'VETRAI_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const VETRAI_API_KEY = 'VETRAI_API_KEY';
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            const client = new VetraiClient({
                baseUrl: VETRAI_SERVER_ADDRESS,
                apiKey: VETRAI_API_KEY
            });

            console.log(`Connecting to Vetrai server at: ${VETRAI_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your Vetrai server is running and accessible at:', VETRAI_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting Vetrai Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting Vetrai Agent...

    Connecting to Vetrai server at: http://localhost:7860
