...
You can export flows to transfer them between Vetrai instances, share them with others, or create backups.
...
* **Export with the Vetrai API**: To export one flow, use the [`/flows/download`](/api-flows#export-flows) endpoint.
...
For more information, see [Vetrai JSON file contents](#vetrai-json-file-contents).
...
If your key is stored in a Vetrai global variable, **Save with my API keys** exports only the variable name.
:::

When you or another user import the flow to another Vetrai instance, that instance must have Vetrai global variables with the same names and valid values in order to run the flow successfully.
If any variables are missing or invalid, those variables must be created or edited after importing the flow.

## Import a flow

You can import Vetrai JSON files from your local machine in the following ways:

* **Import to projects**: On the **Projects** page, click <Icon name="Upload" aria-hidden="true"/> **Upload a flow**, and then select the Vetrai JSON file to import.
* **Import anywhere**: Drag and drop Vetrai JSON files from your file explorer into your Vetrai window to import a flow from any Vetrai page.
* **Import with the Vetrai API**: To import one Vetrai JSON file, use the [`/flows/upload/`](/api-flows#import-flows) endpoint.
To import a zip archive of Vetrai JSON files, use the [`/projects/upload`](/api-projects#import-a-project) endpoint.

### Run an imported flow

Once imported, your flow is ready to use.
If the flow contains any global variables, make sure your Vetrai instance has global variables with the same names and valid values.
For more information, see [Save with my API keys](/concepts-flows-import#save-with-my-api-keys).

## Vetrai JSON file contents

An exported flow is downloaded to your local machine as a JSON file named `FLOW_NAME.json`.

Vetrai JSON files contain the following:
...
For example Vetrai JSON files, you can examine any of the https://github.com/vetrai-ai/vetrai/tree/main/src/backend/base/vetrai/initial_setup/starter_projects[templates in the Vetrai repository], or you can create a flow from a template in Vetrai, export it, and then open the exported JSON file in a text editor.
