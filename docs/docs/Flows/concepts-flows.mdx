...
Vetrai flows are fully serializable and can be saved and loaded from the file system where Vetrai is installed.

:::tip
To try building and running a flow in a few minutes, see the [Vetrai quickstart](/get-started-quickstart).
:::
...
* **Create a flow from a template**: Select a project, click **New Flow**, and then click the template that you want to use.
    ...
    You can also [contribute templates](/contributing-templates) to the Vetrai codebase.
...
You can also create a flow with the [Vetrai API](/api-flows), but the Vetrai team recommends using the [visual editor](/concepts-overview) until you are familiar with flow creation.
...
When you're ready to use Vetrai for application development, learn how to [trigger flows with the Vetrai API](/concepts-publish), explore more advanced configuration options like [custom dependencies](/install-custom-dependencies), and, eventually, [containerize your Vetrai application](/develop-application).

When you're ready to go to production or deploy a Vetrai MCP server for access over the public internet, see [Vetrai deployment overview](/deployment-overview).

#### Flow graphs

When a flow runs, Vetrai builds a Directed Acyclic Graph (DAG) object from the nodes (components) and edges (connections), and the nodes are sorted to determine the order of execution.
...
The **Projects** page is where you arrive when you launch Vetrai.
From here, you can manage flows and your projects' [MCP servers](/mcp-server).

Vetrai projects are like folders that you can use to organize related flows.
...
:::tip
To get back to the **Projects** page after editing a flow, click the project name or Vetrai icon in the Vetrai header.
:::
...
By default, flows and flow execution data are stored in the Vetrai database, and flow logs are stored with other Vetrai logs in the Vetrai config directory.
