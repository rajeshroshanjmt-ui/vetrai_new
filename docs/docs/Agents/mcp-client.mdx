---
title: Use Vetrai as an MCP client
slug: /mcp-client
---
...
This page describes how to use Vetrai as an MCP client with the [**MCP Tools** component](#use-the-mcp-tools-component) and connected [MCP servers](#manage-connected-mcp-servers).

For information about using Vetrai as an MCP server, see [Use Vetrai as an MCP server](/mcp-server).

## Use the MCP tools component

The **MCP Tools** component connects to an MCP server so that a [Vetrai agent](/agents) can use the server's tools when responding to user queries.

This component has two modes, depending on the type of server you want to access:

* [Connect to a non-Vetrai MCP server](#mcp-stdio-mode) with a JSON configuration file, server start command, or HTTP/SSE URL to access tools provided by external, non-Vetrai MCP servers.
* [Connect to a Vetrai MCP server](#mcp-http-mode) to use flows from your [Vetrai projects](/concepts-flows#projects) as MCP tools.

### Connect to a non-Vetrai MCP server {#mcp-stdio-mode}
...
2. In the **MCP Server** field, select a previously connected server or click <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**.
    ...
    * **HTTP/SSE**: Enter your MCP server's **Name**, **URL**, and any **Headers** and **Environment Variables** the server uses, and then click **Add Server**.
    The default **URL** for Vetrai MCP servers is `http://localhost:7860/api/v1/mcp/project/PROJECT_ID/streamable` or `http://localhost:7860/api/v1/mcp/streamable`. For more information, see [Connect to a Vetrai MCP server](#mcp-http-mode).
...
3. To use environment variables in your server command, enter each variable in the **Env** fields as key-value pairs.
    ...
    :::tip
    Vetrai passes environment variables from the `.env` file to MCP, but it doesn't pass global variables declared in your Vetrai **Settings**.
    To define an MCP server environment variable as a global variable, add it to Vetrai's `.env` file at startup.
...
### Connect a Vetrai MCP server {#mcp-http-mode}

Every Vetrai project runs a separate MCP server that exposes the project's flows as MCP tools.
For more information about your projects' MCP servers, including exposing flows as MCP tools, see [Use Vetrai as an MCP server](/mcp-server).
...
2. In the **MCP URL** field, enter your Vetrai server's MCP endpoint.
   ...
   - Default for Vetrai Desktop: `http://localhost:7868/`
...
To manage all MCP server connections for your Vetrai client, click <McpIcon /> **MCP servers** in the visual editor, or click your profile icon, select **Settings**, and then click **MCP Servers**.
