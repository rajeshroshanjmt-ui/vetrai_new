...
By default, [Vetrai agents](/agents) only include the functionality built-in to their base LLM.
...
2. Enter Python code into the **Code** pane to create the custom component.
    ...
    from vetrai.schema import Data
    import re

    class TextAnalyzerComponent(Component):
        display_name = "Text Analyzer"
        description = "Analyzes and transforms input text."
        documentation: str = "http://docs.vetrai.org/components/custom"
        icon = "chart-bar"
...
Vetrai supports **Tool Mode** for the following data types:
