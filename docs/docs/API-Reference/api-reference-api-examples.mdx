---
title: Get started with the Vetrai API
...
You can use the Vetrai API for programmatic interactions with Vetrai, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build Vetrai as a dependency of a larger application, codebase, or service.
* Contribute to the overall Vetrai codebase.

To view and test all available endpoints, you can access the Vetrai API's OpenAPI specification at your Vetrai deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the Vetrai API in a script, see the [Vetrai quickstart](/get-started-quickstart).
...
## Form Vetrai API requests

While individual options vary by endpoint, all Vetrai API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a Vetrai API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

```bash
curl --request POST \
  --url "$VETRAI_SERVER_URL/api/v1/run/$FLOW_ID?stream=false" \
...
By default, local deployments serve the Vetrai API at `http://localhost:7860/api`.

Remotely hosted Vetrai deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:VETRAI_PORT/api`.

You can configure the Vetrai port number in the `VETRAI_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:VETRAI_PORT/api`

### Authentication

In Vetrai versions 1.5 and later, most API endpoints require authentication with a Vetrai API key in either an `x-api-key` header or query parameter.
...
Vetrai API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The Vetrai API serves `/v1` and `/v2` endpoints.
...
```bash
# Set environment variables
export VETRAI_API_KEY="sk..."
export VETRAI_SERVER_URL="https://localhost:7860"
...
Commonly used values in Vetrai API requests include your [Vetrai server URL](#base-url), [Vetrai API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some Vetrai API requests

Once you have your Vetrai server URL, try calling these endpoints that return Vetrai metadata.

### Health check

Returns the health status of the Vetrai database and chat services:

```bash
curl -X GET \
  "$VETRAI_SERVER_URL/health_check" \
...
Vetrai provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before Vetrai is fully initialized, so it's not reliable for checking Vetrai service health.

### Get version

Returns the current Vetrai API version:

```bash
curl -X GET \
  "$VETRAI_SERVER_URL/api/v1/version" \
  -H "accept: application/json"
```
...
Returns configuration details for your Vetrai deployment.
Requires a [Vetrai API key](/api-keys-and-authentication).

```bash
curl -X GET \
  "$VETRAI_SERVER_URL/api/v1/config" \
  -H "accept: application/json" \
...
Returns a dictionary of all Vetrai components.
Requires a [Vetrai API key](/api-keys-and-authentication).

```bash
curl -X GET \
  "$VETRAI_SERVER_URL/api/v1/all" \
  -H "accept: application/json" \
  -H "x-api-key: $VETRAI_API_KEY"
```
...
Because you can run Vetrai as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the Vetrai database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the Vetrai codebase, you won't directly call most of the Vetrai endpoints.
...
The following endpoints are useful for developing applications with Vetrai and administering Vetrai deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return Vetrai version. See [Get version](/api-reference-api-examples#get-version).
...
* Custom components: You might use these endpoints when developing custom Vetrai components for your own use or to share with the Vetrai community:
  * GET `/v1/all`: Return all available Vetrai component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the Vetrai codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* MCP servers: The following endpoints are for managing Vetrai MCP servers and MCP server connections.
They aren't typically called directly; instead, they are used to drive internal functionality in the Vetrai frontend and when running flows that call MCP servers.
Vetrai MCP servers support both streamable HTTP and SSE transport.
...
* Use the Vetrai API to [run a flow](/api-flows-run).
* Use the Vetrai API to [upload files](/api-files).
* Use the Vetrai API to [get flow logs](/api-logs).
* Explore all endpoints in the [Vetrai API specification](/api).