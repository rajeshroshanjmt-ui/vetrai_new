...
Vetrai includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).
...
To be compatible with Vetrai's OpenAI Responses API endpoint, your flow and request must adhere to the following requirements:
...
Vetrai uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to Vetrai with OpenAI client libraries, you must configure custom headers and include an `api_key` configuration.
...
In the following examples, replace the values for `VETRAI_SERVER_URL`, `VETRAI_API_KEY`, and `FLOW_ID` with values from your deployment.
<Tabs groupId="client">
...
client = OpenAI(
  base_url="VETRAI_SERVER_URL/api/v1/",
  default_headers={"x-api-key": "VETRAI_API_KEY"},
  api_key="dummy-api-key" # Required by OpenAI SDK but not used by Vetrai
)
...
const client = new OpenAI({
  baseURL: "VETRAI_SERVER_URL/api/v1/",
  defaultHeaders: {
    "x-api-key": "VETRAI_API_KEY"
  },
  apiKey: "dummy-api-key" // Required by OpenAI SDK but not used by Vetrai
});
...
```bash
curl -X POST \
  "$VETRAI_SERVER_URL/api/v1/responses" \
...
|--------|----------|-------------|---------|
| `x-api-key` | Yes | Your Vetrai API key for authentication | `"sk-..."` |
| `Content-Type` | Yes | Specifies the JSON format | `"application/json"` |
| `X-VETRAI-GLOBAL-VAR-*` | No | Global variables for the flow | `"X-VETRAI-GLOBAL-VAR-API_KEY: sk-..."` For more, see [Pass global variables to your flows in headers](#global-var). |
...
The response contains fields that Vetrai sets dynamically and fields that use OpenAI-compatible defaults.
...
Fields set dynamically by Vetrai:
...
```bash
curl -X POST \
  "http://$VETRAI_SERVER_URL/api/v1/responses" \
  -H "x-api-key: $VETRAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "$FLOW_ID",
...
The `/responses` endpoint accepts global variables as custom HTTP headers with the format `X-VETRAI-GLOBAL-VAR-{VARIABLE_NAME}`.
...
This example demonstrates passing an `OPENAI_API_KEY` variable, which is a variable Vetrai automatically detects from environment variables, with two custom variables for `USER_ID` and `ENVIRONMENT`. The variables don't have to be created in Vetrai's Global Variables section - you can pass any variable name in the `X-VETRAI-GLOBAL-VAR-{VARIABLE_NAME}` header format.
...
curl -X POST \
  ...
  -H "x-api-key: $VETRAI_API_KEY" \
  -H "Content-Type: application/json" \
  -H "X-VETRAI-GLOBAL-VAR-OPENAI_API_KEY: sk-..." \
  -H "X-VETRAI-GLOBAL-VAR-USER_ID: user123" \
  -H "X-VETRAI-GLOBAL-VAR-ENVIRONMENT: production" \
  -d '{
    "model": "your-flow-id",
    "input": "Hello"
  }'
...
Variables passed with `X-VETRAI-GLOBAL-VAR-{VARIABLE_NAME}` are always available to your flow, regardless of whether they exist in the database.

If your flow components reference variables that aren't provided in headers or your Vetrai database, the flow fails by default.
