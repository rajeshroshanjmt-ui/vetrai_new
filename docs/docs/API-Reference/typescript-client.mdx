---
title: Vetrai TypeScript client
...
The Vetrai TypeScript client allows your TypeScript applications to programmatically interact with the Vetrai API.

For the client code repository, see [vetrai-client-ts](https://github.com/datastax/vetrai-client-ts/).

For the npm package, see [@datastax/vetrai-client](https://www.npmjs.com/package/@datastax/vetrai-client).

## Install the Vetrai TypeScript package

To install the Vetrai typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/vetrai-client
```
...
```bash
yarn add @datastax/vetrai-client
```
...
```bash
pnpm add @datastax/vetrai-client
```

</TabItem>
</Tabs>

## Initialize the Vetrai TypeScript client

1. Import the client into your code.

    ```tsx
    import { VetraiClient } from "@datastax/vetrai-client";
    ```

2. Initialize a `VetraiClient` object to interact with your server:

    ```tsx
    const baseUrl = "BASE_URL";
    const apiKey = "API_KEY";
    const client = new VetraiClient({ baseUrl, apiKey });
    ```

    Replace `BASE_URL` and `API_KEY` with values from your deployment.
    The default Vetrai base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## Connect to your server and get responses

1. With your Vetrai client initialized, test the connection by calling your Vetrai server.
    ...
    const client = new VetraiClient({ baseUrl });
    ...
    * `baseUrl`: The URL of your Vetrai server.
...
2. Review the result to confirm that the client connected to your Vetrai server.

    The following example shows the response from a well-formed `runFlow` request that reached the Vetrai server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with Vetrai:
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming Vetrai responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve Vetrai logs with the TypeScript client

To retrieve [Vetrai logs](/logging), you must enable log retrieval on your Vetrai server by including the following values in your Vetrai `.env` file:

```text
VETRAI_ENABLE_LOG_RETRIEVAL=True
VETRAI_LOG_RETRIEVER_BUFFER_SIZE=10000
VETRAI_LOG_LEVEL=DEBUG
```
...
async function runFlow(client: VetraiClient) {
    const input = "Is anyone there?";
...
async function main() {
    const client = new VetraiClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <vetrai.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
