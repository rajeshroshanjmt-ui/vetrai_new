...
Retrieve logs for your Vetrai flows and server.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your Vetrai instance.

To enable log retrieval, set the following [environment variables](/environment-variables) in your Vetrai `.env` file, and then start Vetrai with `uv run vetrai run --env-file .env`:

```text
VETRAI_ENABLE_LOG_RETRIEVAL=True
VETRAI_LOG_RETRIEVER_BUFFER_SIZE=10000 // Must be greater than 0
VETRAI_LOG_LEVEL=DEBUG // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
...
```bash
curl -X GET \
  "$VETRAI_URL/logs?lines_before=0&lines_after=0&timestamp=0" \
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/vetrai/profile_pictures/Space.\n"
}
```
