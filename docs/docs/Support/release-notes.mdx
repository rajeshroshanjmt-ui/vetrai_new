...
This page summarizes significant changes to Vetrai in each release.
For all changes, see the [Changelog](https://github.com/vetrai-ai/vetrai/releases/latest).

Due to strict SemVer requirements, Vetrai Desktop can have different patch versions than the core Vetrai OSS Python package, but the major and minor versions are aligned.

## Prepare to upgrade

:::warning
Whenever possible, the Vetrai team recommends installing new Vetrai versions in a new virtual environment or VM before upgrading your primary installation.
...
To avoid the impact of potential breaking changes and test new versions, the Vetrai team recommends the following upgrade process:

1. Recommended: [Export your projects](/api-projects#export-a-project) to create backups of your flows:

    ```bash
    curl -X GET \
    "$VETRAI_SERVER_URL/api/v1/projects/download/$PROJECT_ID" \
...
2. Install the new version:
   ...
   * **Vetrai OSS Python package**: Install the new version in a new virtual environment. For instructions, see [Install and run the Vetrai OSS Python package](/get-started-installation#install-and-run-the-vetrai-oss-python-package).
   * **Vetrai Docker image**: Run the new image in a separate container.
   * **Vetrai Desktop**: To upgrade in place, open Vetrai Desktop, and then click **Upgrade Available** in the Vetrai header. If you want to isolate the new version, you must install Vetrai Desktop on a separate physical or virtual machine, and then [import your flows](/concepts-flows-import) to the new installation.
...
For all changes, see the [Changelog](https://github.com/vetrai-ai/vetrai/releases).
...
All Vetrai versions upgrading to 1.7.0 are affected.

Don't upgrade to Vetrai version 1.7.0.
...
1. Revert Vetrai to version 1.6.9:

   ```bash
   uv pip install vetrai==1.6.9
   ```
...
- Support for streamable HTTP transport for MCP clients and servers
    ...
    Vetrai now supports [streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) for both MCP clients and servers.
    When using Vetrai as an [MCP client](/mcp-client), you can connect to MCP servers with streamable HTTP transport.
    When using Vetrai as an [MCP server](/mcp-server), clients can connect using streamable HTTP transport.
    SSE transport is still supported as a fallback for backwards compatibility.

- Webhook authentication

    Added the `VETRAI_WEBHOOK_AUTH_ENABLE` environment variable for authenticating requests to the [`/webhook` endpoint](/api-flows-run#webhook-run-flow). When `VETRAI_WEBHOOK_AUTH_ENABLE=TRUE`, webhook endpoints require API key authentication and validate that the authenticated user owns the flow being executed. When `FALSE`, no Vetrai API key is required and all requests to the webhook endpoint are treated as being sent by the flow owner. For more information, see [Trigger flows with webhooks](/webhook).

- Configurable API key validation

    Added the `VETRAI_API_KEY_SOURCE` environment variable to control how Vetrai validates API keys. When set to `db`, Vetrai validates API keys against keys stored in the database. When set to `env`, Vetrai validates API keys against the `VETRAI_API_KEY` environment variable. For more information, see [API keys and authentication](/api-keys-and-authentication#vetrai-api-key-source).

- SSRF protection

    Added SSRF (Server-Side Request Forgery) protection to the [**API Request** component](/api-request). HTTP redirects are disabled by default to prevent SSRF bypass attacks. To enable SSRF protection, set `VETRAI_SSRF_PROTECTION_ENABLED=TRUE`. Configure allowed hosts with `VETRAI_SSRF_ALLOWED_HOSTS`. Flows that relied on automatic redirects will need to enable it manually.

- Email registration in Vetrai Desktop

    Vetrai Desktop now includes an email registration screen at startup. The registered email address helps track user registrations and understand the user base. For more information, see [Telemetry](/contributing-telemetry#email).
...
Vetrai versions 1.6.0 through 1.6.3 have a critical bug where environment variables from `.env` files aren't read.
...
If your `.env` file includes `AUTO_LOGIN=false`, upgrading to the impacted versions causes Vetrai to fall back to default settings, potentially giving all users superuser access immediately upon upgrade.
Additionally, database credentials, API keys, and other sensitive configurations can't be loaded from `.env` files.

_Don't_ upgrade to any Vetrai version from 1.6.0 through 1.6.3 if you use `.env` files for configuration.
...
:::warning
Windows users of Vetrai Desktop should **not** use the in-app update feature to upgrade to Vetrai version 1.6.0.
Only Windows Desktop users upgrading to Vetrai version 1.6.0 are affected.
:::

The **Update** button in Vetrai Desktop will not work for Windows users, and may result in data loss.

Instead, download a fresh installation from the [Vetrai website](https://vetrai.org) when a new version is available.

Follow the instructions below to minimize the risk of losing flows.

These instructions assume the default path for the Vetrai database of `C:\Users\YOUR_USERNAME\AppData\Roaming\com.vetrai\data\database.db` or `C:\Users\%YOUR_USERNAME%\AppData\Roaming\com.vetrai\data\database.db`. Replace `YOUR_USERNAME` with your Windows username.

1. Ensure you have Administrator privileges.
2. Ensure you have enough disk space for a second installation of Vetrai.
3. Close Vetrai, and ensure no Vetrai process is running in Task Manager.
4. The Vetrai 1.6.0 installer automatically performs a database backup in a later step, but an additional manual backup provides additional redundancy and is recommended.
To manually back up your Vetrai database file, do the following:
   1. In Windows Explorer, navigate to `C:\Users\YOUR_USERNAME\AppData\Roaming\com.vetrai\data\`. Replace `YOUR_USERNAME` with your Windows username.
   2. Copy the `database.db` at this location, and paste it to a safe location.
5. Download the Windows installer from the [Vetrai website](https://vetrai.org).
...
The Vetrai 1.6.0 installer automatically performs a database backup.
These steps install Vetrai from scratch, and result in two Vetrai installations: the previously installed version, and version 1.6.0.
This is expected behavior.
8. Start version 1.6.0 of Vetrai, and confirm your flows behave as expected.
9. **If flows are missing**, restore your flows from your manual backup by doing the following:
    ...
    4. Start Vetrai, and confirm your flows behave as expected.
    5. After confirmation, uninstall the previous version of Vetrai, and keep version 1.6.0.

### Breaking changes

- Authentication enforced for Vetrai API requests by default

    In Vetrai version 1.6, `VETRAI_AUTO_LOGIN=True` and `VETRAI_SKIP_AUTH_AUTO_LOGIN=False` by default.
    This enforces authentication for Vetrai API requests while still automatically authenticating all users as superusers in the visual editor.
    ...
    **This is a breaking change from 1.5 where both of these environment variables were `true` by default, bypassing all authentication.**
    ...
    For more information, see the documentation for [`VETRAI_AUTO_LOGIN`](/api-keys-and-authentication#vetrai-auto-login).

### New features and enhancements

- OpenAI Responses API compatibility

  Vetrai now includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses) at `POST /api/v1/responses`.
...
- Advanced document parsing with built-in Docling support
  ...
  To make it easier to use the [**Docling** components](/bundles-docling) and the **Read File** component's new advanced parsing feature, the Docling dependency is now included with Vetrai for all operating systems except macOS Intel (x86_64).
...
- Vetrai API requests can require authentication

    To enhance security and ensure proper authentication for automatic login features, Vetrai API endpoints now require authentication with a Vetrai API key, even when `VETRAI_AUTO_LOGIN=True`.
    This change will be enforced in a future release.
    For temporary backwards compatibility, this release adds the `VETRAI_SKIP_AUTH_AUTO_LOGIN` environment variable.
    The default value is `true`, which disables API authentication enforcement.
    To enforce API authentication, set `VETRAI_SKIP_AUTH_AUTO_LOGIN=False`.
    For more information, see the documentation for [`VETRAI_AUTO_LOGIN`](/api-keys-and-authentication#vetrai-auto-login).
...
- MCP server one-click installation
    ...
    On your Vetrai project's **MCP server** page, click **Auto install** to install your Vetrai MCP server to MCP clients with just one click.
    The option to install with a JSON configuration file is available for macOS, Windows, and WSL.
    For more information, see [Use Vetrai as an MCP server](/mcp-server).

- MCP server management

    You can now add, remove, and edit your MCP servers in the **MCP Tools** components and through your Vetrai **Settings** page.
    For more information, see [Use Vetrai as an MCP client](/mcp-client).
...
- Stability improvements
    ...
    See an issue? [Raise it on GitHub](https://github.com/vetrai-ai/vetrai/issues).
...
- Added an enhanced **Breaking Changes** feature to help update components without breaking flows after updating Vetrai.
...
- Introduced MCP server functionality to serve Vetrai tools to MCP-compatible clients.
...
See the [Changelog](https://github.com/vetrai-ai/vetrai/releases).