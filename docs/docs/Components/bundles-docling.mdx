...
Vetrai integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing and chunking documents.
...
The Docling dependency is required to use the Docling components in Vetrai.

    * **Vetrai version 1.6 and later**: The Docling dependency is included by default for all operating systems except macOS Intel (x86_64).
    ...
    * **Earlier versions**: Vetrai versions earlier than 1.6 don't include the Docling dependency.
    For Vetrai OSS, install the Docling extra with `uv pip install 'vetrai[docling]'`.
    For Vetrai Desktop, add the Docling dependency to Vetrai Desktop's `requirements.txt`.
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + Vetrai: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
