...
:::tip
Vetrai provides keyboard shortcuts for the workspace.

In the Vetrai header, click your profile icon, select **Settings**, and then click **Shortcuts** to view the available shortcuts.
:::
...
* <Icon name="Component" aria-hidden="true" /> **Core components**: Vetrai's base components are grouped by purpose, such as **Inputs and Outputs** or **Data**.
...
For information about the programmatic representation of each data type, see [Vetrai data types](/data-types).
...
For example, the [**Recursive Character Text Splitter** component](https://github.com/vetrai-ai/vetrai/blob/main/src/lfx/src/lfx/components/langchain_utilities/recursive_character.py) is a child of the [`LCTextSplitterComponent`](https://github.com/vetrai-ai/vetrai/blob/main/src/lfx/src/lfx/base/textsplitters/model.py) class.
...
Component versions and states are stored in an internal Vetrai database. When you add a component to a flow, you create a detached copy of the component based on the information in the Vetrai database.
These copies are detached from the primary Vetrai database, and they don't synchronize with any updates that can occur when you upgrade your Vetrai version.
...
When editing a flow in the workspace, Vetrai notifies you if a component's workspace version is behind the database version so you can update the component's workspace version:
...
Components are updated to the latest available version, based on the version of Vetrai you are running.
