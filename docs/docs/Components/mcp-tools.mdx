...
The **MCP Tools** component connects to a Model Context Protocol (MCP) server and exposes the MCP server's functions as tools for Vetrai agents to use to respond to input.

In addition to publicly available MCP servers and your own custom-built MCP servers, you can connect Vetrai MCP servers, which allow your agent to use your Vetrai flows as tools.
To do this, use the **MCP Tools** component's [HTTP/SSE mode](/mcp-client#mcp-http-mode) to connect to your Vetrai project's MCP server.

For more information, see [Use Vetrai as an MCP client](/mcp-client) and [Use Vetrai as an MCP server](/mcp-server).
...
* In Vetrai version 1.5, the **MCP Connection** component was renamed to the **MCP Tools** component.
* In Vetrai version 1.3, the **MCP Tools (stdio)** and **MCP Tools (SSE)** components were removed and replaced by the unified **MCP Connection** component, which was later renamed to **MCP Tools**.
