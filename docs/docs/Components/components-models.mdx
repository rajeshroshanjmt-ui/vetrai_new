...
Language model components in Vetrai generate text using a specified Large Language Model (LLM).
These components accept inputs like chat messages, files, and instructions in order to generate a text response.

Vetrai includes a **Language Model** core component that has built-in support for many LLMs.
