...
:::info
Voice mode is not available in Vetrai Desktop.
To use voice mode, [Install the Vetrai OSS Python package](/get-started-installation#install-and-run-the-vetrai-oss-python-package).
:::

You can use Vetrai's voice mode to interact with your flows verbally through a microphone and speakers.
...
* An [OpenAI](https://platform.openai.com/) account and an OpenAI API key because Vetrai uses the OpenAI API to process voice input and generate responses.
...
5. Enter your OpenAI API key, and then click **Save**. Vetrai saves the key as a [global variable](/configuration-global-variables).
...
Vetrai saves this key as a global variable.
...
Vetrai exposes two OpenAI Realtime API-compatible websocket endpoints for your flows.
...
The Vetrai API's websockets endpoints require an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and they support an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
...
This is the mode used in the Vetrai **Playground**.
...
If omitted, Vetrai uses the flow ID as the [session ID](/session-id).
