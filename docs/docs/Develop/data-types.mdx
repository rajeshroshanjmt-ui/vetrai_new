...
Vetrai components are designed to accept and produce specific types of inputs and outputs.
...
The schema is defined in [`data.py`](https://github.com/vetrai-ai/vetrai/blob/main/src/backend/base/vetrai/schema/data.py).
...
The schema is defined in [`dataframe.py`](https://github.com/vetrai-ai/vetrai/blob/main/src/backend/base/vetrai/schema/dataframe.py).

The following attributes are available:

- **Full pandas compatibility**: All pandas DataFrame methods and functionality are supported

- **Vetrai integration**: Accepts lists of [`Data` objects](#data), dictionaries, or existing DataFrames.
...
Because Vetrai is built on LangChain, `LanguageModel` is actually an instance of a [LangChain chat model](https://docs.langchain.com/oss/python/integrations/chat) that uses the configuration parameters set in the originating component.
...
The `Message` schema is defined in [`message.py`](https://github.com/vetrai-ai/vetrai/blob/main/src/backend/base/vetrai/schema/message.py).
Some `Message` attributes have their own schema definitions, such as [`content_block.py`](https://github.com/vetrai-ai/vetrai/blob/main/src/backend/base/vetrai/schema/content_block.py).
...
When using the Vetrai API, the response includes the `Message` object along with other response data from the flow run.
Vetrai API responses can be extremely verbose, so your applications must include code to extract relevant data from the response to return to the user.
For an example, see the [Vetrai quickstart](/get-started-quickstart).
...
For more information, see [Configure tools for agents](/agents-tools) and [Use Vetrai as an MCP client](/mcp-client).
...
In Vetrai, you can use <Icon name="TextSearch" aria-hidden="true" /> **Inspect output** to view the output of individual components.
