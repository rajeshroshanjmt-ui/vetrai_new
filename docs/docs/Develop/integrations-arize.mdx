...
To enable Arize tracing, set the required Arize environment variables in your Vetrai deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating Vetrai and Arize are also available in the Arize documentation:

* [Vetrai tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/vetrai/vetrai-tracing)
* [Vetrai tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/vetrai/vetrai-tracing)
:::
...
2. In the root of your Vetrai application, edit your existing Vetrai `.env` file or create a new one.
...
4. Start your Vetrai application with your `.env` file:
...
1. In Vetrai, run a flow that has an LLM-driven component, such as an **Agent** component or any language model component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each Vetrai execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the Vetrai components.
