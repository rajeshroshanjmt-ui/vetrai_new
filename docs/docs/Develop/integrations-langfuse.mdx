...
- A [running Vetrai server](/get-started-installation) with a [flow](/concepts-flows) that you want to trace

:::tip
If you need a flow to test the Langfuse integration, see the [Vetrai quickstart](/get-started-quickstart).
:::
...
3. Set your Langfuse project credentials as environment variables in the same environment where you run Vetrai.
...
## Start Vetrai and view traces in Langfuse

1. Start Vetrai in the same environment where you set the Langfuse environment variables:
...
2. Run a flow.
    ...
    Vetrai automatically collects and sends tracing data about the flow execution to Langfuse.
...
To disable the Langfuse integration, remove the [Langfuse environment variables](#langfuse-credentials), and then restart Vetrai.

## Run Langfuse and Vetrai with Docker Compose
...
3. Add your Vetrai credentials to your Vetrai `docker-compose.yml` file in the `environment` section.

    The following example is based on the [example `docker-compose.yml`](https://github.com/vetrai-ai/vetrai/blob/main/docker_example/docker-compose.yml).

    ```yml
    services:
      vetrai:
        image: vetraiai/vetrai:latest # or another version tag on https://hub.docker.com/r/vetraiai/vetrai
...
5. To confirm Langfuse is connected to your Vetrai container, run the following command:

    ```sh
    docker compose exec vetrai python -c "import requests, os; addr = os.environ.get('LANGFUSE_HOST'); print(addr); res = requests.get(addr, timeout=5); print(res.status_code)"
    ```
